...
| El panel de la campana se coloca midiendo, no con `right: 0` | Era `position: absolute; right: 0; width: 340px`: anclado al borde derecho de la campana y creciendo hacia la izquierda. En un teléfono la campana está a ~200px del borde, así que el panel empezaba en −140 y media lista quedaba fuera. `max-width` no lo arregla — el problema no es el ancho, es de qué lado crece. `calcularPosicion()` conserva la alineación a la derecha cuando cabe y empuja hasta el margen cuando no. Al pasar a `fixed` el panel deja de alargar la página, así que también acota el alto: si no, se cortaría por abajo en vez de por la izquierda. |

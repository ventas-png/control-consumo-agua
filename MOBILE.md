...
| Si el WebView no popula `env(safe-area-inset-top)`, la app se sale de `viewport-fit=cover` sola | `asegurarSafeAreaTop()` (`src/lib/nativeApp.ts`) mide el inset con una sonda en el primer frame. Toda la fila anterior depende de que ese valor llegue: si es 0, los `calc()` se quedan en 8px/16px y los chips vuelven a caer bajo el reloj. Con `viewport-fit=contain` es el motor quien encaja el viewport dentro del área segura, así que no queda nada que reservar. Cuando el inset sí llega, la función no toca nada. |

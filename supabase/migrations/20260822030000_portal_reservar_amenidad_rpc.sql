-- Portal: reserva de amenidades del residente vía RPC ATÓMICO (con cargo a la unidad).
--
-- El flujo del portal insertaba la cuota del cargo DESDE EL NAVEGADOR
-- (PortalReservasTab.hacerReserva): cuotas_condominio es staff-only por RLS
-- (20260518000009), así que el camino "cargar a la unidad" estaba roto para TODOS
-- los residentes; el monto viajaba del cliente; la pareja cuota→reserva no era
-- atómica (compensación best-effort en el navegador); y el chequeo de choques era
-- CIEGO — el residente solo ve sus propias reservas (RLS SELECT + fetch del
-- portal), podía doble-reservar encima de otra unidad. Anotado como fuera de
-- alcance en 20260822010000; esta migración lo cierra:
--
--   • portal_reservar_amenidad(): valida TODO server-side (reglas de
--     lib/amenidadesReglas.ts, bloqueos, conflicto GLOBAL bajo FOR UPDATE de la
--     amenidad) y calcula la TARIFA en el servidor; si aplica "cargar_unidad" y
--     la reserva queda confirmada, crea la cuota 'amenidad' en la MISMA
--     transacción.
--   • portal_cancelar_reserva(): cancela con guardas explícitas y anula
--     (soft-delete) la cuota solo si sigue pendiente — una pagada no se toca.
--   • Se quita la rama residente de reservas_amenidades INSERT/UPDATE
--     (20260822010000): con el RPC como único camino, dejarla viva permitiría
--     insertar por PostgREST una reserva confirmada con tarifa_pagada=true
--     saltándose el cálculo del servidor. El SELECT dual no se toca.
--   • uq_cuotas_condominio_llave_natural excluye concepto 'amenidad': los cargos
--     de amenidad son POR EVENTO (varios por mes son legítimos); el índice
--     (20260717080000, pensado para el generador de cuotas MENSUALES) rompía la
--     2ª reserva con cargo del mes y los flujos admin aprobarReserva()/
--     retenerDeposito(). Relajación pura: no puede fallar sobre datos existentes,
--     y el generador mensual nunca emite 'amenidad' — su garantía queda íntegra.
--
-- Horas = reloj de pared 'America/Guatemala' (espeja el reloj del navegador que
-- usaba validarReglasAmenidad; multi-TZ exigiría timezone por proyecto).

-- ── 1) Índice único de cuotas: los cargos por evento quedan fuera ────────────
DROP INDEX IF EXISTS public.uq_cuotas_condominio_llave_natural;
CREATE UNIQUE INDEX uq_cuotas_condominio_llave_natural
  ON public.cuotas_condominio (unidad_id, periodo, concepto)
  WHERE deleted_at IS NULL AND unidad_id IS NOT NULL AND concepto <> 'amenidad';
COMMENT ON INDEX public.uq_cuotas_condominio_llave_natural IS
  'Clave natural anti-duplicado de la generación de cuotas MENSUALES (unidad+período+concepto). Excluye ''amenidad'' (cargos por evento: varios por mes son legítimos — 20260822030000). Parcial: soft-deleted no bloquea regeneración.';

-- ── 2) portal_reservar_amenidad ──────────────────────────────────────────────
...
BEGIN
  ...
  IF v_amen.horas_minimas_antelacion IS NOT NULL AND v_amen.horas_minimas_antelacion > 0
     AND (p_fecha + p_hora_inicio) < v_ahora + make_interval(hours => v_amen.horas_minimas_antelacion) THEN
...
-- ── 3) portal_cancelar_reserva ───────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.portal_cancelar_reserva(p_reserva_id uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $fn$
DECLARE
  v_res public.reservas_amenidades%ROWTYPE;
  v_hoy date := (now() AT TIME ZONE 'America/Guatemala')::date;
  v_n   int := 0;
BEGIN
  -- Lock de la fila: dos cancelaciones concurrentes serializan aquí.
  SELECT * INTO v_res FROM public.reservas_amenidades WHERE id = p_reserva_id FOR UPDATE;
  IF NOT FOUND THEN
    -- Mensaje único a propósito: no revela si la reserva existe.
    RAISE EXCEPTION 'Reserva no encontrada o no autorizada.';
  END IF;
  -- Residente de la unidad (dual) o staff del tenant (vista previa del portal),
  -- mismo criterio que portal_reservar_amenidad.
  IF current_user_role() = 'cliente' THEN
    IF NOT EXISTS (SELECT 1 FROM public.mis_unidades_ids() x WHERE x = v_res.unidad_id) THEN
      RAISE EXCEPTION 'Reserva no encontrada o no autorizada.';
    END IF;
  ELSIF NOT (is_super_admin()
             OR (v_res.company_id = get_my_company_id()
                 AND user_has_permission('condominios.tab.amenidades'))) THEN
    RAISE EXCEPTION 'Reserva no encontrada o no autorizada.';
  END IF;
  IF v_res.estado NOT IN ('pendiente', 'confirmada') THEN
    RAISE EXCEPTION 'La reserva ya no se puede cancelar (estado %).', v_res.estado;
  END IF;
  IF v_res.checkin_at IS NOT NULL THEN
    RAISE EXCEPTION 'La reserva ya tiene check-in; contacta a la administración.';
  END IF;
  IF v_res.fecha < v_hoy THEN
    RAISE EXCEPTION 'No se puede cancelar una reserva pasada.';
  END IF;

  UPDATE public.reservas_amenidades SET estado = 'cancelada' WHERE id = v_res.id;

  IF v_res.cuota_id IS NOT NULL THEN
    -- Soft-delete SOLO si sigue pendiente y viva (mismo criterio que el
    -- softDelete del cliente, pero atómico y sin fallar en silencio). Una cuota
    -- pagada NO se toca: el reembolso es decisión de la administración.
    UPDATE public.cuotas_condominio
       SET deleted_at = now(), deleted_by = auth.uid()
     WHERE id = v_res.cuota_id AND estado = 'pendiente' AND deleted_at IS NULL;
  ...
  RETURN jsonb_build_object(
    ...
    'estado', 'cancelada',
    'cuota_anulada', v_n = 1,
    'cuota_id', v_res.cuota_id
  );
END;
$fn$;

REVOKE ALL ON FUNCTION public.portal_cancelar_reserva(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.portal_cancelar_reserva(uuid) TO authenticated;
COMMENT ON FUNCTION public.portal_cancelar_reserva(uuid) IS
  'El residente cancela una reserva propia (pendiente/confirmada, futura, sin check-in) y anula (soft-delete) la cuota asociada si sigue pendiente. Errores explícitos vía RAISE; una cuota pagada no se toca.';

-- ── 4) reservas_amenidades: la escritura del residente pasa SOLO por los RPCs ─
-- Se retira la rama residente de INSERT/UPDATE (20260822010000): dejarla viva
-- permitiría saltarse el cálculo server-side de la tarifa por PostgREST directo
-- (p.ej. insertar confirmada con tarifa_pagada=true). Staff conserva exactamente
-- el alcance vigente; para clientes get_my_company_id() es NULL ⇒ ambas ramas
-- cerradas. El SELECT dual del residente NO se toca.

...
-- 20260904000000 dejó a propósito con `area_id = NULL` toda programación de
...
-- 20260904000000 para `programacion_limpieza.area` (snapshot, no fuente).
--
...
-- 20260904000000 para las áreas que creó su backfill.
...
BEGIN
  ...
  ELSE
    ...
    --    RESTRICT de programacion_limpieza.area_id (20260904000000) revienta
...
-- ────────────────────────────────────────────────────────────────────────────
-- Indexable porque `areas_normalizar_nombre` es IMMUTABLE (20260904000000).
...
-- ────────────────────────────────────────────────────────────────────────────
-- Mismo UPDATE que 20260904000000: coincidencia ÚNICA por nombre normalizado

-- Fase 3: Complete RLS Policy Refactoring
-- Simplifies 15+ complex RLS policies using helper functions
-- Expected benefit: +5-10% additional RLS performance improvement
-- All changes maintain identical security logic and functionality
...
DROP POLICY IF EXISTS "operator read tarifas (old)" ON public.tarifas;
...
DROP POLICY IF EXISTS "cliente access on conversations" ON public.conversations;
  ...
  USING (
    EXISTS (
      ...
      WHERE c.id = conversation_messages.conversation_id
        AND (has_super_admin_access() OR has_admin_company_access(c.company_id) OR has_company_owner_company_access(c.company_id))
    )
  )
  WITH CHECK (
    EXISTS (
...
CREATE POLICY "cliente access on conversation_messages" ON public.conversation_messages
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.conversations c
      WHERE c.id = conversation_messages.conversation_id
        AND c.cliente_id = get_my_cliente_id()

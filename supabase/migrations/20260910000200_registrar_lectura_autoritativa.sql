...
-- 20260910000300, para que la corrección sea una decisión humana.
--
-- REVERSIÓN
...
COMMENT ON COLUMN public.registros.secuencia IS
  'Posición de la lectura en la cadena de SU contador, asignada bajo el bloqueo. Es el orden total de verdad: `fecha` empata (dos lecturas del mismo día caen ambas al mediodía) y `created_at` también, porque es now() y vale lo mismo para todo lo que entre en la misma transacción. NULL en las filas anteriores a 20260910000200, que se ordenan como se pueda (por eso existe el reporte de inconsistencias).';
...
-- el reporte de 20260910000300 lo audita igual, sin importar el origen.

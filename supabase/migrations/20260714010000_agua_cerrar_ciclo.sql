-- Cierre de ciclo para AGUA — espejo de condominios_cerrar_ciclo (20260713110000).
--
-- CONTEXTO: condominios ya tiene cierre de ciclo (emisión masiva por período +
-- aviso in_app/email al residente). Agua no: la emisión de facturas de agua era
-- SOLO por fila (useEmitirFacturaMutation) o un lote client-side secuencial
-- (CobrosSection.emitirLoteSeleccion), sin RPC set-based y sin aviso al residente.
-- Este RPC cierra esa brecha con el MISMO patrón que condominios.
--
-- MODELO (clave): en agua NO hay tabla de factura aparte — la "Factura" son
-- columnas sobre `registros` (migración 20260604160000): factura_estado (máquina
-- de estados), emitida_at/…, iva_*, mora_*, total_a_pagar. Un `registro` es a la
-- vez la lectura y el cargo. Diferencias vs cuotas_condominio:
--   • `registros` NO tiene company_id → la empresa se resuelve por project_id →
--     projects.company_id (este RPC es POR PROYECTO, como el de condominios).
--   • `registros` NO tiene columna `periodo` → el período se ancla en `fecha`
--     (timestamptz, NOT NULL) por rango de mes [YYYY-MM-01, +1 mes). La BD corre
--     en UTC (verificado), así que el rango coincide con el mes civil.
--   • Agua SÍ lleva IVA (condominios no): el snapshot de emisión calcula
--     iva_tasa/iva_monto/monto_con_iva/total_a_pagar.
--   • El residente se enlaza DIRECTO por registros.cliente_id (sin unidad/rol).
--
-- PARIDAD EXACTA con la emisión por fila (buildEmitirFacturaPatch + business.ts):
--   • emitible = normalizarEstadoFactura(COALESCE(factura_estado, estado)) =
--     'pendiente' (igual que CobrosSection: `factura_estado ?? r.estado`).
--     Normalización legacy: 'pagado'→pagada, 'mora'→vencida, NULL/otro→pendiente.
--   • patch: factura_estado='emitida', emitida_at=now, fecha_vencimiento =
--     (now + días)::date con días = reglas_mora_config activa del proyecto ?? 30,
--     y el desglose de calcularTotalFactura(monto_calculado, ivaTasa, mora):
--        base          = round(monto_calculado, 2)  (0 si <= 0)
--        iva_tasa      = companies.iva_tasa_default acotada a [0,1] (NULL → 0.12 GT)
--        iva_monto     = round(base * iva_tasa, 2)
--        monto_con_iva = round(base + iva_monto, 2)
--        mora          = round(mora_monto, 2)  (0 si <= 0)
--        total_a_pagar = round(monto_con_iva + mora, 2)
--   • Aviso al cliente responsable vía el outbox (mismo pipeline real que
--     condominios): in_app SIEMPRE + email si el tenant tiene Gmail (inerte si no;
--     respeta el opt-out por notification_preferences vía payload.user_id).
--     NO se setea `ruta_id` (un registro no es una ruta; user_notifications.ruta_id
--     es FK a rutas) — se usa `registro_id` como referencia.
--
-- AUTORIZACIÓN (patrón condominios_cerrar_ciclo): SECURITY DEFINER con guard
-- interno is_super_admin() OR (empresa del proyecto = get_my_company_id() AND
-- user_has_permission('agua.cobros.change_status')), 42501 si no. IS NOT TRUE para
-- fail-closed también con auth.uid() NULL. REVOKE de PUBLIC/anon; GRANT a authenticated.
--
-- Validado contra prod vía BEGIN…ROLLBACK con claims simulados de un company_owner.
-- Apply por el gate humano P0 #9.
...
COMMENT ON FUNCTION public.agua_cerrar_ciclo IS
  'Cierra el ciclo de facturación de agua de un proyecto/período: emite en un solo statement todos los registros emitibles del mes (paridad con buildEmitirFacturaPatch: factura_estado=emitida + vencimiento por regla de mora + snapshot de IVA/total) y opcionalmente avisa al cliente vía el outbox por in_app (siempre) y email (si el tenant tiene Gmail; respeta el opt-out). Staff gated: super admin o empresa del proyecto + permiso agua.cobros.change_status.';

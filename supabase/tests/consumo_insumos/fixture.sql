-- Fixture para EJECUTAR 20260907000500 (consumo de insumos) contra un Postgres
-- de verdad. El run.sh aplica encima las migraciones REALES 20260904000100,
-- 000100, 000200, 20260907000200, 000300 y 000500.
--
-- QUÉ SE DECLARA AQUÍ Y POR QUÉ
-- Sólo el ESQUEMA previo y el padrón. Las tareas de prueba viven en seed.sql,
-- que corre DESPUÉS de las migraciones porque necesitan `tarea_bloque_suministros`.
--
-- `suministros_condominio`, `movimientos_suministro` y sus DOS triggers de stock
-- se declaran aquí LITERALES de 20260821000200 en vez de aplicar esa migración:
-- aquélla arrastra todo el módulo de compras (órdenes, recepciones, folios) que
-- no tiene nada que ver con lo que se prueba. Lo que importa es que el motor de
-- existencias sea EL MISMO — que `salida` reste con piso en 0 y que el UPDATE
-- directo a `stock_actual` se revierta— porque la migración nueva se apoya en
-- él y no lo toca. Mismo criterio con el que este fixture ya copia
-- `sellar_actor`, `sellar_cierre` y `assert_company_scope`.
--
-- `inventario_condominio` está porque 20260904000300 le crea un ancla UNIQUE y
-- le cuelga `plantilla_tarea_herramientas`; no se prueba aquí.
--
-- `auth.uid()` se emula leyendo un GUC de sesión (`app.uid`), igual que en
-- supabase/tests/turnos/fixture.sql. OJO: `set_config(…, true)` es LOCAL A LA
-- TRANSACCIÓN, y cada bloque DO de psql es su propia transacción — la identidad
-- se vuelve a declarar en cada bloque que invoque la RPC.
...
-- este punto es `tarea_bloque_suministros`, y 20260907000500 hace su propio

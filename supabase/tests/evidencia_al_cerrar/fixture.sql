-- Fixture para EJECUTAR 20260907000400 (evidencia al cerrar) contra un Postgres
-- de verdad. El run.sh aplica encima las migraciones REALES 20260904000100,
-- 000100, 20260907000200, 000300 y 000400.
--
-- Aquí sólo va el ESQUEMA y el padrón: las tareas de prueba viven en seed.sql,
-- que corre DESPUÉS de las migraciones, porque usan columnas
-- (`requiere_comentario`, `checklist`) que las crea 20260907000300.
--
-- `bloques_turno` y `tareas_bloque` se declaran YA con lo que les dejó
-- 20260907000100 (paridad): aplicar esa migración exigiría media serie de
...
-- LOS DOS TRIGGERS QUE 20260907000100 YA DEJÓ SOBRE ESTA TABLA, copiados
-- literalmente de esa migración. Sin ellos el sandbox probaría
-- `trg_exigir_evidencia` EN SOLEDAD, y en producción no corre solo: son tres
-- BEFORE UPDATE FOR EACH ROW sobre la misma tabla y PostgreSQL los dispara en
-- ORDEN ALFABÉTICO, así que el nuevo va PRIMERO —antes de que el sellado
-- escriba completada_en/completado_por—. Esa interacción es justo lo que hay
-- que ver: que rechazar no impide sellar lo que sí pasa, y que sellar no
-- esquiva el rechazo.
...
INSERT INTO public.app_users (id, full_name, company_id, role) VALUES
  ('a0000000-0000-0000-0000-00000000000a', 'Operativo',

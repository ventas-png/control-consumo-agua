-- Fixture para EJECUTAR 20260912020300 (la vara de la jornada) contra un
...
-- devuelva EXACTAMENTE lo mismo antes y después de declarar la vara. Sin
-- `calcular_horas_personal` de verdad no se puede comprobar eso.
--
-- Respecto a aquel fixture agrega lo que la política necesita de la plantilla:
-- `minutos_descanso` (que ya alimenta `horas_planificadas`) y el permiso del tab
-- de turnos, que es el que gobierna los cupos.
...
CREATE TABLE public.bloques_turno (
  ...
  -- de laboratorio: es el que más veces toca esta tabla, y el que no puede
  -- refrescar la vara congelada.
  estado               text,
  notas                text
);
...
INSERT INTO public.test_permisos (user_id, permission_key) VALUES
  ...
  -- Y el tab de TURNOS, que es el que gobierna la jornada y sus cupos. Marco

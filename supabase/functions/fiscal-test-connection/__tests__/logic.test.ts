...
describe('fiscal-test-connection/hayCredenciales (override locación→empresa)', () => {
  it('credenciales con al menos un ambiente cuentan', () => {
    expect(hayCredenciales({ sandbox: { usuario: 'x' } })).toBe(true)
    expect(hayCredenciales({ prod: { token: 't' } })).toBe(true)
  })

  it('objeto vacío / null / undefined NO tapan las credenciales de la empresa', () => {
    expect(hayCredenciales({})).toBe(false)
    expect(hayCredenciales(null)).toBe(false)
    expect(hayCredenciales(undefined)).toBe(false)
  })
})

...
export function buildConfigUpdates(
  ...
  publicKey: string,
  /**
   * El secreto YA CIFRADO. El cifrado en reposo (P0 #7) lo hace el handler con
   * `encryptSecret()` de _shared/secretsCrypto.ts, que no puede vivir aquí sin
   * romper la pureza de este módulo. Se llama así a propósito: si alguien pasa
   * el texto plano, la credencial se guarda sin cifrar y nada lo delata.
   */
  ...
  if (provider === 'stripe') {
    return {
      ...
      secretsUpdate: {
        ...
        stripe_secret_key: secretCifrado,
        updated_at: nowIso,
      },
    }
  }
  return {
    ...
    secretsUpdate: {
      company_id: companyId,
      paypal_client_secret: secretCifrado,
      updated_at: nowIso,
    },
  }
}

...
describe('stripe-platform-webhook/buildSubscriptionRow', () => {
  // Guarda del P0 #2: el reloj de dunning lo calcula el handler (es quien ve la
  // fila existente), pero el row DEBE transportarlo. Si alguien vuelve a
  // extraer este helper sin el parámetro, past_due_since se perdería en silencio
  // y la ventana de gracia de company_write_enabled dejaría de funcionar.
  it('transporta el past_due_since que le pasa el handler', () => {
    const sello = '2026-03-01T00:00:00.000Z'
    expect(buildSubscriptionRow(baseSub, 'co_1', 'p', NOW_MS, sello).past_due_since).toBe(sello)
  })

  it('past_due_since es null cuando el handler no pasa sello', () => {
    expect(buildSubscriptionRow(baseSub, 'co_1', 'p', NOW_MS).past_due_since).toBeNull()
  })

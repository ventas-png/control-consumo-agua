...
describe('google-oauth-callback/buildEmailConfigRecord', () => {
  ...
  it('scope de tenant: company_id del state e is_superadmin false (nunca se mezclan)', () => {
    const rec = buildEmailConfigRecord(tokens, 'gerencia@mayan.com', { company_id: 'emp-1', is_superadmin: false }, NOW)
    expect(rec).toEqual({
      provider: 'google',
  ...
  it('scope superadmin: company_id null + is_superadmin true (aunque el state traiga empresa)', () => {
    const rec = buildEmailConfigRecord(tokens, 'plataforma@admin.com', { company_id: 'emp-colada', is_superadmin: true }, NOW)
  ...
  it('sin refresh_token (re-consent de Google) → null explícito, no undefined', () => {
    const rec = buildEmailConfigRecord({ access_token: 'at-2', expires_in: 60 }, 'a@b.com', { company_id: 'emp-1', is_superadmin: false }, NOW)

...
describe('process-email-queue/isCronAuthorized', () => {
  it('autoriza solo cuando el header coincide con el secret configurado', () => {
    expect(isCronAuthorized('s3cr3t', 's3cr3t')).toBe(true)
    expect(isCronAuthorized('otro', 's3cr3t')).toBe(false)
    expect(isCronAuthorized(null, 's3cr3t')).toBe(false)
  })

  it('FAIL-CLOSED: sin CRON_SECRET configurado nadie queda autorizado (ni con header vacío)', () => {
    expect(isCronAuthorized('', '')).toBe(false)
    expect(isCronAuthorized(null, '')).toBe(false)
    expect(isCronAuthorized('cualquier-cosa', '')).toBe(false)
  })

  it('la comparación es exacta (sin prefijos/espacios que cuelen)', () => {

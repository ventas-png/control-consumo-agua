-- ════════════════════════════════════════════════════════════════════════════
-- ANDAMIAJE «COMO UNA SUPABASE BRANCH DE VERDAD» — sin el regalo de funciones
-- ════════════════════════════════════════════════════════════════════════════
-- Es `bootstrap.sql` con UNA diferencia, y esa diferencia es el punto entero
-- de este archivo: NO trae
--
--     ALTER DEFAULT PRIVILEGES IN SCHEMA public
--       GRANT ALL ON FUNCTIONS TO postgres, anon, authenticated, service_role;
--
-- POR QUÉ. Ese `ALTER DEFAULT PRIVILEGES` hace que toda función creada después
-- nazca con `authenticated=X` en su ACL. En la reconstrucción del auditor eso
-- es lo correcto: reproduce lo que hay en producción. Pero también TAPÓ un
-- defecto real durante semanas — `20260909000000` revoca EXECUTE de PUBLIC
-- sobre 15 helpers de RLS dando por hecho un GRANT a `authenticated` que
-- NINGUNA migración del repositorio concede. Con el regalo puesto, la
-- reconstrucción pasaba. Sin él —que es el caso de una Supabase Branch nueva—
-- la propia verificación de esa migración aborta:
--
...
--
-- Se vio el 2026-09-11 al recrear la preview branch de #847 (proyecto
-- iiohmlctjtgqdxigdtjr), no en el repositorio. Este archivo existe para que la
-- prueba de replay pueda verlo SIN salir de un Postgres desechable.
--
-- NO se usa para comparar huellas: el auditor de drift sigue con `bootstrap.sql`
-- en sus tres lados. Mezclarlos mediría el andamiaje y no el esquema.
--
-- Se mantiene sincronizado a mano con `bootstrap.sql`; la única diferencia
-- permitida es la de arriba, y hay una prueba que lo comprueba.
-- ════════════════════════════════════════════════════════════════════════════
...
-- (A PROPÓSITO NO ESTÁ: GRANT ALL ON FUNCTIONS — ver la cabecera de este archivo.)
ALTER DEFAULT PRIVILEGES IN SCHEMA public

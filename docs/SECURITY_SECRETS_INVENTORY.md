...
| `ALLOW_VERCEL_PREVIEW_ORIGINS` | Flag de seguridad | helper CORS compartido | Abre CORS a los previews de Vercel **de este proyecto y este equipo** (patrón anclado en `_shared/cors.ts`). **Sólo en el proyecto SANDBOX**: un preview es código sin revisar, y en producción sería un origen permitido capaz de leer respuestas autenticadas de clientes reales. Ausente o con cualquier valor que no sea `true` ⇒ previews rechazados (fail-closed). **Estado verificado 2026-09-10:** presente en el sandbox `jwpmivhvlstslncrtokb`, ausente en producción `nnsqmeigtgewatameexo` — comprobado por listado y, además, por comportamiento (en prod un Origin de preview recibe el fallback). Cómo re-verificarlo sin medir humo: `e2e/README.md`. |

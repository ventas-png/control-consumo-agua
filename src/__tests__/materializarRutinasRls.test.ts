...
// Guards ESTÁTICOS de la materialización de rutinas (20260905000300).
//
...
const MIG_MAT = '20260905000300_materializar_rutinas.sql'
...
describe('20260905000300 · la SECURITY DEFINER trae su propio control', () => {
  ...
  it('además exige el permiso del tab, y sólo los dos que corresponden', () => {
    ...
    // El mismo par que acepta el INSERT de tareas_bloque en 20260905000100:
...
describe('20260905000300 · la tarea COPIA, no lee por join', () => {
  const SNAPSHOT = [
...
describe('20260905000300 · idempotencia que respeta las decisiones', () => {
  ...
  it('el índice que sostiene esa idempotencia sigue declarado en 20260905000100', () => {
    // Si alguien lo quita allá, aquí el ON CONFLICT deja de tener a qué agarrarse.
    const paridad = soloCodigo(
      readFileSync(join(MIGRATIONS_DIR, '20260905000100_tareas_bloque_paridad.sql'), 'utf8'),
    )
...
describe('20260905000300 · el guard de append-only sigue contento', () => {
  it('es la última migración de la serie y nadie la reescribió después', () => {
    ...
    // para que este guard se disparara, y 20260905000500 la nombra dentro de un

...
/** Los 31 días para el selector de `mensual_dias`. */
...
export function reglaAplicaEn(
  ...
  switch (regla.frecuencia) {
    ...
    case 'mensual_dias': {
      ...
      // meses.
      //
      // FIN DE MES. Un día que ese mes no tiene se recorta al último real, con
      // el mismo criterio que las periodicidades de día fijo: saltarse el mes
      // dejaría al empleado sin turno siete veces al año. Y como esto es un
      // `some` sobre la lista —no un elemento por día—, que el 29, el 30 y el 31
      // converjan en el 28 de febrero produce UN día, no tres.
      //
      ...
      const marcados = (regla.dias_mes ?? []).filter(d => Number.isInteger(d))
      if (marcados.length === 0) {
        return dia === Math.min(regla.dia_mes ?? partes(regla.fecha_inicio).d, ultimo)
      }
      return marcados.some(d => Math.min(Math.max(d, 1), ultimo) === dia)
    }
...
export function celdaDe(
  ...
  // Una excepción SIN bloque deja el día vacío: la regla ya no predice nada ahí.
  // Con bloque manda el bloque —quien reasignó el día después de quitarlo
  // decidió lo contrario— y la excepción sólo queda como rastro.
 ...
 *
 * ESTA FUNCIÓN NO ES LA AUTORIDAD. La autoridad es la base: el trigger
 * `trg_turnos_bloque_borrable` (20260916171325) exige, para CUALQUIER rol, que
 * el bloque esté pendiente, sin iniciar, sin cerrar, con fecha de hoy en
 * adelante EN LA ZONA DE LA EMPRESA, y sin tareas, revisiones ni marcajes. La
 * UI no puede comprobar las tres últimas —no tiene esas filas a mano— ni la
 * zona del inquilino, así que acá sólo se filtra lo que sí se sabe, para no
 * ofrecer un botón que la base va a rechazar. Lo que se cuele, lo rechaza la
 * base y el mensaje se muestra tal cual.
 *
 ...
 *     ya ocurrió es trabajo de Presencia, que deja rastro de quién cambió qué.
 *   · LO YA EMPEZADO TAMPOCO. Un bloque en curso, completado o incompleto
 *     arrastra su checklist y sus revisiones.
 *
...
export function celdaEditable(celda: CeldaTurno, hoy: string): boolean {
  ...
  if (!celda.bloque) return true
  return celda.bloque.estado === 'pendiente'
    && !celda.bloque.iniciado_en
    && !celda.bloque.cerrado_en
}

/** Por qué no se puede editar esta celda. `null` = hasta donde la UI sabe, sí. */
export function motivoNoEditable(celda: CeldaTurno, hoy: string): string | null {
  ...
  if (celda.bloque?.iniciado_en) return 'El turno ya arrancó: se corrige desde Presencia, no aquí.'
  if (celda.bloque?.cerrado_en) return 'El turno ya se cerró: se corrige desde Presencia, no aquí.'
  if (celda.bloque && celda.bloque.estado !== 'pendiente') {
    return `El turno está en estado «${celda.bloque.estado}»: solo se editan los pendientes.`
  }
  return null
}

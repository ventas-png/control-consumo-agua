...
function Badge({ v, invertido = false }: { v: ReturnType<typeof variacion>; invertido?: boolean }) {
  if (v.dir === 'eq') return <span style={{ fontSize: 10, color: '#9ca3af' }}>—</span>
...
export default function ComparativoAnualTab({ cuotas, gastos, moneda }: Props) {
  ...
  const SEL = { padding: '6px 10px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 13, background: '#fff', fontWeight: 600 }

  return (
    <div style={{ padding: 16 }}>
      <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', marginBottom: 2 }}>Comparativo Anual</div>

      {/* Selectores */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <select value={anioA} onChange={e => setAnioA(Number(e.target.value))} style={{ ...SEL, color: '#2563eb' }}>
          {aniosDisponibles.map(a => <option key={a} value={a}>{a}</option>)}
        </select>
        <span style={{ fontWeight: 700, color: '#9ca3af' }}>vs</span>
        <select value={anioB} onChange={e => setAnioB(Number(e.target.value))} style={{ ...SEL, color: '#7c3aed' }}>
          {aniosDisponibles.map(a => <option key={a} value={a}>{a}</option>)}
        </select>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        {[
          ...
          return (
            <div key={k.label} style={{ flex: '1 1 150px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '10px 14px' }}>
              <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 6 }}>{k.label}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 4 }}>
                <div>
                  <div style={{ fontSize: 9, color: '#2563eb', fontWeight: 600 }}>{anioA}</div>
                  <div style={{ fontSize: 16, fontWeight: 800, color: '#0f172a' }}>{moneda} {Math.round(k.a).toLocaleString('es')}</div>
                </div>
                <Badge v={v} invertido={k.inv} />
              </div>
              <div style={{ fontSize: 10, color: '#7c3aed' }}>{anioB}: {moneda} {Math.round(k.b).toLocaleString('es')}</div>
            </div>
          )
        })}
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 14, marginBottom: 14 }}>
        <div style={{ fontWeight: 700, fontSize: 12, color: '#0f172a', marginBottom: 10 }}>Ingresos vs Egresos por mes</div>
            ...
            return (
              <div key={mes} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                <div style={{ width: '100%', display: 'flex', gap: 1, alignItems: 'flex-end', height: 110 }}>
                  {/* Año A — ingresos */}
                  <div style={{ flex: 1, background: '#2563eb', opacity: 0.85, borderRadius: '2px 2px 0 0',
                  ...
                  {/* Año B — ingresos */}
                  <div style={{ flex: 1, background: '#7c3aed', opacity: 0.6, borderRadius: '2px 2px 0 0',
                ...
                <div style={{ fontSize: 8, color: '#9ca3af' }}>{mes}</div>
              </div>
            )
          })}
        </div>
        <div style={{ display: 'flex', gap: 14, marginTop: 8, fontSize: 10, flexWrap: 'wrap' }}>
          {[
            { color: '#2563eb', opacity: '0.85', label: `${anioA} Ingresos` },
            { color: '#f97316', opacity: '0.85', label: `${anioA} Egresos` },
            { color: '#7c3aed', opacity: '0.6', label: `${anioB} Ingresos` },
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr style={{ background: '#f8fafc' }}>
              <th style={{ padding: '8px 10px', textAlign: 'left', color: '#64748b', fontWeight: 600 }}>Mes</th>
              <th colSpan={3} style={{ padding: '8px 10px', textAlign: 'center', color: '#2563eb', fontWeight: 700, borderRight: '2px solid #e5e7eb' }}>{anioA}</th>
              <th colSpan={3} style={{ padding: '8px 10px', textAlign: 'center', color: '#7c3aed', fontWeight: 700, borderRight: '2px solid #e5e7eb' }}>{anioB}</th>
              <th style={{ padding: '8px 10px', textAlign: 'center', color: '#64748b', fontWeight: 600 }}>Δ Ingresos</th>
            </tr>
            <tr style={{ background: '#f8fafc', borderTop: '1px solid #f1f5f9' }}>
              <th style={{ padding: '4px 10px' }} />
              <th style={{ padding: '4px 10px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: 10 }}>Ingresos</th>
              <th style={{ padding: '4px 10px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: 10 }}>Egresos</th>
              <th style={{ padding: '4px 10px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: 10, borderRight: '2px solid #e5e7eb' }}>Saldo</th>
              <th style={{ padding: '4px 10px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: 10 }}>Ingresos</th>
              <th style={{ padding: '4px 10px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: 10 }}>Egresos</th>
              <th style={{ padding: '4px 10px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: 10, borderRight: '2px solid #e5e7eb' }}>Saldo</th>
              <th style={{ padding: '4px 10px' }} />
          ...
          <tbody>
            {MESES.map((mes, i) => {
              ...
              return (
                <tr key={mes} style={{ borderTop: '1px solid #f1f5f9', opacity: noData ? 0.4 : 1 }}>
                  <td style={{ padding: '7px 10px', fontWeight: 600, color: '#374151' }}>{mes}</td>
                  ...
                  <td style={{ padding: '7px 10px', textAlign: 'right', fontWeight: 700, color: mA.superavit >= 0 ? '#16a34a' : '#ef4444', borderRight: '2px solid #e5e7eb' }}>
                    {(mA.ingresos > 0 || mA.egresos > 0) ? `${mA.superavit >= 0 ? '+' : ''}${moneda} ${Math.round(mA.superavit).toLocaleString('es')}` : '—'}
                  </td>
                  <td style={{ padding: '7px 10px', textAlign: 'right', color: '#7c3aed' }}>{mB.ingresos > 0 ? `${moneda} ${Math.round(mB.ingresos).toLocaleString('es')}` : '—'}</td>
                  <td style={{ padding: '7px 10px', textAlign: 'right', color: '#9333ea' }}>{mB.egresos > 0 ? `${moneda} ${Math.round(mB.egresos).toLocaleString('es')}` : '—'}</td>
                  <td style={{ padding: '7px 10px', textAlign: 'right', fontWeight: 700, color: mB.superavit >= 0 ? '#7c3aed' : '#ef4444', borderRight: '2px solid #e5e7eb' }}>
          ...
          <tfoot>
            <tr style={{ borderTop: '2px solid #e5e7eb', background: '#f8fafc' }}>
              ...
              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 800, color: totalesA.superavit >= 0 ? '#16a34a' : '#ef4444', borderRight: '2px solid #e5e7eb' }}>
                {totalesA.superavit >= 0 ? '+' : ''}{moneda} {Math.round(totalesA.superavit).toLocaleString('es')}
              </td>
              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, color: '#7c3aed' }}>{moneda} {Math.round(totalesB.ingresos).toLocaleString('es')}</td>
              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700, color: '#9333ea' }}>{moneda} {Math.round(totalesB.egresos).toLocaleString('es')}</td>
              <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 800, color: totalesB.superavit >= 0 ? '#7c3aed' : '#ef4444', borderRight: '2px solid #e5e7eb' }}>

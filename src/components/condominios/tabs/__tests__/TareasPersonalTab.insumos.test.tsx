// El insumo que se usa deja de ser gratis (20260907000500).
//
// EL AGUJERO QUE CIERRA. `plantilla_tarea_suministros` declaraba qué insumos
// lleva cada actividad y su propio COMMENT lo decía: «Planificación pura: sin
// consumo ni descuento de stock.» El conserje usaba el cloro y el inventario
// seguía diciendo que estaba lleno, así que `stock_minimo` nunca disparaba
// reposición.
//
...
// mande como 0 y no se omita, que el consumo NO ocurra si el cierre falló, y
// que la falta de existencias se avise como advertencia y no como error —el
// insumo se gastó igual y la tarea ya cerró.
...
const mocks = vi.hoisted(() => ({
  ...
  consumirInsumosTarea: vi.fn(async () => ({
...
vi.mock('../../../../domain/condominios/tabMutations', () => ({
  ...
  consumirInsumosTarea: mocks.consumirInsumosTarea,
}))
...
function insumo(over: Partial<TareaBloqueSuministro> = {}): TareaBloqueSuministro {
  return {
    ...
    movimiento_id: null, created_at: '2026-01-01T00:00:00.000Z',
    ...over,
  }
}

/** Monta y despliega el turno; los bloques vienen colapsados. */
function montar(insumos: TareaBloqueSuministro[]) {
 ...
 */
async function montarConPanel(insumos: TareaBloqueSuministro[]) {
  montar(insumos)
  fireEvent.click(await screen.findByLabelText('Evidencia de Limpiar la piscina'))
}

/** Los consumos que se mandaron a la RPC en la primera llamada. */
function consumosEnviados(): { suministro_id: string; cantidad: number }[] {
  const [, consumos] = mocks.consumirInsumosTarea.mock.calls[0] as unknown as
    [string, { suministro_id: string; cantidad: number }[]]
  return consumos
}
...
beforeEach(() => {
  ...
  mocks.consumirInsumosTarea.mockResolvedValue({
...
describe('TareasPersonalTab · lo que se manda a descontar', () => {
  it('manda la cantidad DECLARADA, no la planificada', async () => {
    ...
    await waitFor(() => expect(mocks.consumirInsumosTarea).toHaveBeenCalled())
    expect(consumosEnviados()).toEqual([{ suministro_id: 'sum1', cantidad: 5 }])
  })

  it('el 0 se manda como 0 y no se omite', async () => {
    // Omitir y declarar 0 son cosas distintas: el 0 es «no lo necesité» y deja
    // constancia en la fila. Si se filtrara, quedaría pendiente para siempre.
    await montarConPanel([insumo()])
  ...
  it('no manda lo que ya se descontó', async () => {
...
describe('TareasPersonalTab · cuándo se consume y cuándo no', () => {
  it('NO consume si el cierre falló', async () => {
    // Si el trigger de evidencia rechaza la tarea, no se gastó nada del
    // almacén. Consumir igual dejaría el inventario mintiendo sobre trabajo
    // que no se cerró.
    mocks.updateCondominioRow.mockResolvedValue({ error: { message: 'EVIDENCIA: falta foto' } })
    await montarConPanel([insumo()])
    fireEvent.click(screen.getByTitle('Completada'))

    await waitFor(() => expect(mocks.notify).toHaveBeenCalled())
    expect(mocks.consumirInsumosTarea).not.toHaveBeenCalled()
  })

  it('omitir no consume: no se hizo el trabajo', async () => {
    await montarConPanel([insumo()])
  ...
  it('una tarea sin insumos no llama a la RPC', async () => {
    // Sin plan no hay panel que abrir: se cierra desde la tarjeta y no debe
    // haber ni un viaje a la RPC.
    montar([])
    fireEvent.click(screen.getByTitle('Completada'))

  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>Plantillas de tareas por cargo</h2>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Cargo *</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Título de la tarea *</label>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', padding: '8px', background: '#f8fafc', borderRadius: '8px', border: '1.5px solid #e2e8f0' }}>
                {ICONOS.map(ic => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div>
                <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Área (opcional)</label>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Descripción / instrucciones</label>
        ...
        {(['todos', ...cargos] as string[]).map(c => (
          <button key={c} onClick={() => setFiltroCargo(c)}
            style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '12.5px', fontWeight: 700, cursor: 'pointer', border: 'none',
              background: filtroCargo === c ? '#fef3c7' : '#f1f5f9', color: filtroCargo === c ? '#d97706' : '#64748b' }}>
      ...
      {plantillas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '56px', color: '#94a3b8' }}>
          ...
          <p style={{ fontWeight: 700, color: '#64748b', marginBottom: '4px' }}>Sin plantillas</p>
          ...
          {cargosPorGrupo.map(([cargo, tareas]) => (
            <div key={cargo}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '12px', color: '#94a3b8' }}>{tareas.length} tarea{tareas.length !== 1 ? 's' : ''}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '10px' }}>
                {tareas.map(p => {
                  const areaNombre = areas.find(a => a.id === p.area_id)?.nombre
                  return (
                    <div key={p.id} style={{ background: 'white', border: `1.5px solid ${p.activo ? '#e2e8f0' : '#f1f5f9'}`, borderRadius: '12px', padding: '14px', opacity: p.activo ? 1 : 0.55 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: '13.5px', color: '#0f172a' }}>{p.titulo}</div>
                          {p.descripcion && <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>{p.descripcion}</div>}
                          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '5px' }}>
                            {areaNombre && <span style={{ fontSize: '11.5px', color: '#64748b' }}>📍 {areaNombre}</span>}
                            {p.requiere_foto && <span style={{ fontSize: '11.5px', color: '#7c3aed' }}>📷 Requiere foto</span>}
                            <span style={{ fontSize: '11.5px', color: '#94a3b8' }}>#{p.orden}</span>
                          </div>
                        </div>
                      </div>
                      {canEdit && (
                        <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
                          <button onClick={() => startEdit(p)} style={{ flex: 1, padding: '5px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: '#374151', fontWeight: 600 }}>✏️ Editar</button>

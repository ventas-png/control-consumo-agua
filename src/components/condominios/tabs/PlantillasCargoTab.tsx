  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#15291F' }}>Plantillas de tareas por cargo</h2>
        ...
        {(['todos', ...cargos] as string[]).map(c => (
          <button key={c} onClick={() => setFiltroCargo(c)}
            style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '12.5px', fontWeight: 700, cursor: 'pointer', border: 'none',
              background: filtroCargo === c ? '#fef3c7' : '#EAE6D8', color: filtroCargo === c ? '#d97706' : '#7E9389' }}>
      ...
      {plantillas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '56px', color: '#7E9389' }}>
          ...
          <p style={{ fontWeight: 700, color: '#7E9389', marginBottom: '4px' }}>Sin plantillas</p>
          ...
          {cargosPorGrupo.map(([cargo, tareas]) => (
            <div key={cargo}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '12px', color: '#7E9389' }}>{tareas.length} tarea{tareas.length !== 1 ? 's' : ''}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '10px' }}>
                {tareas.map(p => {
                  const areaNombre = areas.find(a => a.id === p.area_id)?.nombre
                  return (
                    <div key={p.id} style={{ background: 'white', border: `1.5px solid ${p.activo ? '#E1DDD0' : '#EAE6D8'}`, borderRadius: '12px', padding: '14px', opacity: p.activo ? 1 : 0.55 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: '13.5px', color: '#15291F' }}>{p.titulo}</div>
                          {p.descripcion && <div style={{ fontSize: '12px', color: '#7E9389', marginTop: '2px' }}>{p.descripcion}</div>}
                          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '5px' }}>
                            {areaNombre && <span style={{ fontSize: '11.5px', color: '#7E9389' }}>📍 {areaNombre}</span>}
                            {p.requiere_foto && <span style={{ fontSize: '11.5px', color: '#9C5733' }}>📷 Requiere foto</span>}
                            <span style={{ fontSize: '11.5px', color: '#7E9389' }}>#{p.orden}</span>
                          </div>
                        </div>
                      </div>
                      {canEdit && (

    ...
    <div style={{ padding: '20px 24px' }}>
      ...
      <DataTable<SancionCondominio>
        ...
        columns={[
          ...
          { key: 'fecha_emision', header: 'Emisión', sortable: true, hideOnMobile: true,
            render: (s) => <span style={{ color: 'var(--at-ink-3)', fontSize: 12 }}>{s.fecha_emision}</span> },
          { key: 'fecha_vencimiento', header: 'Vencimiento', sortable: true,
            accessor: (s) => s.fecha_vencimiento ?? '',
            render: (s) => {
              const vencida = s.estado === 'pendiente' && s.fecha_vencimiento && s.fecha_vencimiento < today
              return <span style={{ fontSize: 12, fontWeight: vencida ? 700 : 400, color: vencida ? 'var(--at-danger)' : 'var(--at-ink-3)' }}>
          ...
          { key: 'estado', header: 'Estado', sortable: true,
            render: (s) => {
              ...
              return <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 20, background: est?.bg, color: est?.color }}>{est?.label}</span>
            } },
          { key: 'actions', header: '', align: 'right',
            render: (s) => (canEdit && s.estado === 'pendiente') ? (
              <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                <button onClick={(e) => { e.stopPropagation(); cambiarEstado(s.id, 'pagado', '¿Marcar como pagada?') }}
                  style={{ padding: '3px 8px', background: 'var(--at-success-tint)', color: 'var(--at-success)', border: 'none', borderRadius: 5, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Pagado</button>
                <button onClick={(e) => { e.stopPropagation(); cambiarEstado(s.id, 'apelado') }}
                  style={{ padding: '3px 7px', background: 'var(--at-primary-soft)', color: 'var(--at-primary-hover)', border: 'none', borderRadius: 5, fontSize: 11, cursor: 'pointer' }}>Apelar</button>
                <button onClick={(e) => { e.stopPropagation(); cambiarEstado(s.id, 'anulado', '¿Anular sanción?') }}
                  style={{ padding: '3px 7px', background: 'var(--at-chip)', color: 'var(--at-ink-3)', border: 'none', borderRadius: 5, fontSize: 11, cursor: 'pointer' }}>Anular</button>
              </div>
            ) : null },
        ] satisfies DataTableColumn<SancionCondominio>[]}
      />
    </div>
  )
}

...
export function BodegasTab({ bodegas, unidades, proyectoId, companyId, moneda, canCreate, canEdit, onRefresh }: Props) {
  ...
  async function handleSave() {
    if (!form.numero?.trim()) return notify({ variant: 'warning', title: 'Campo requerido', text: 'Ingresa el número de bodega.' })
    setSaving(true)
    const payload = {
  ...
  async function handleDelete(id: string) {
    const r = await confirm({ title: '¿Eliminar bodega?', icon: 'warning', variant: 'danger', confirmText: 'Eliminar' })
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: 'var(--at-ink)' }}>Bodegas / Almacenamiento</h2>
        {canCreate && !showForm && (
          <button onClick={() => setShowForm(true)} style={{ padding: '8px 16px', background: 'var(--at-primary)', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>+ Agregar Bodega</button>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '12px' }}>
          {filtered.map(b => {
            ...
            return (
              <div key={b.id} style={{ background: 'var(--at-surface)', border: '1.5px solid var(--at-line)', borderRadius: '10px', padding: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  <div>
                    <div style={{ fontWeight: 800, fontSize: '18px', color: 'var(--at-ink)' }}>🗄️ {b.numero}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', fontSize: '12px', color: 'var(--at-ink-3)', marginBottom: '10px' }}>
                  ...
                  {b.monto_renta && <div style={{ fontWeight: 600, color: 'var(--at-ink)' }}>💰 {moneda} {b.monto_renta}/mes</div>}
                  {b.fecha_asignacion && <div>📅 Desde {b.fecha_asignacion}</div>}
                </div>
                {canEdit && (
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    {b.estado === 'disponible' && (
                      <button onClick={() => handleEstado(b.id, 'asignada')} style={{ flex: 1, padding: '4px 8px', background: 'var(--at-primary-soft)', color: 'var(--at-primary-hover)', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
                        Asignar
                      </button>
                    )}
                    {b.estado === 'asignada' && (

  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--at-ink)' }}>Comparativo Presupuesto vs. Real</div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div style={{ display: 'flex', background: 'var(--at-chip)', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={k.label} style={{ flex: '1 1 130px', background: k.bg, border: `1px solid ${k.color}22`, borderRadius: 10, padding: '10px 14px' }}>
            <div style={{ fontSize: 10, color: 'var(--at-ink-3)', marginBottom: 2 }}>{k.label}</div>
            ...
            <thead>
              <tr style={{ background: 'var(--at-surface-2)' }}>
                {['Categoría', 'Presupuestado', 'Ejecutado', 'Variación', '% Ejec.', 'Estado', 'Barra'].map(h => (
                  <th key={h} style={{ padding: '10px 12px', textAlign: h === 'Categoría' ? 'left' : 'right', color: 'var(--at-ink-3)', fontWeight: 600, fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filasCat.map((f, idx) => {
                const sem = f.presup === 0 && f.real === 0 ? { bg: 'var(--at-surface-2)', color: 'var(--at-ink-3)', label: '—' } : semaforo(f.pct)
                ...
                return (
                  <tr key={f.cat} style={{ borderTop: idx > 0 ? '1px solid var(--at-chip)' : undefined }}>
                    ...
                    <td style={{ padding: '10px 12px', width: 100 }}>
                      <div style={{ background: 'var(--at-chip)', borderRadius: 4, height: 8, overflow: 'hidden' }}>
                ...
                <div key={f.mes} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 1, height: 100 }}>
                    <div title={`Presupuesto: ${fmt(f.presup, moneda)}`}
                      style={{ width: 10, height: `${hPresup}%`, background: 'var(--at-primary-soft-2)', borderRadius: '3px 3px 0 0', minHeight: 2 }} />
                  ...
                  <div style={{ fontSize: 9, color: 'var(--at-ink-3)' }}>{f.label}</div>
                </div>
              )
            })}
          </div>
            ...
            <thead>
              <tr style={{ background: 'var(--at-surface-2)' }}>
                {['Mes','Presup.','Real','Var.','% Ejec.'].map(h => (
                  <th key={h} style={{ padding: '8px 10px', textAlign: h === 'Mes' ? 'left' : 'right', color: 'var(--at-ink-3)', fontWeight: 600 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>

...
const TIPO_STYLE: Record<string, { bg: string; color: string; label: string }> = {
  cultural:    { bg: '#F4EBE3', color: '#9C5733', label: 'Cultural' },
  deportivo:   { bg: '#dcfce7', color: '#16a34a', label: 'Deportivo' },
  social:      { bg: '#D9E2DC', color: '#102622', label: 'Social' },
  informativo: { bg: '#fef3c7', color: '#92400e', label: 'Informativo' },
  otro:        { bg: '#EAE6D8', color: '#7E9389', label: 'Otro' },
}

const ESTADO_STYLE: Record<string, { bg: string; color: string; label: string }> = {
  programado: { bg: '#D9E2DC', color: '#102622', label: 'Programado' },
  en_curso:   { bg: '#dcfce7', color: '#16a34a', label: 'En curso' },
  realizado:  { bg: '#EAE6D8', color: '#7E9389', label: 'Realizado' },
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: '0 0 2px', fontSize: '16px', fontWeight: 700, color: '#15291F' }}>Eventos Comunitarios</h2>
          <p style={{ margin: 0, fontSize: '12px', color: '#7E9389' }}>{eventos.filter(e => e.estado === 'programado').length} programados · {eventos.filter(e => e.estado === 'realizado').length} realizados</p>
        </div>
        {canCreate && !showForm && (
      ...
      {showForm && (
        <div style={{ background: '#FAF7EF', border: '1.5px solid #E1DDD0', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '3px' }}>Capacidad máx.</label>
              <input style={inputStyle} type="number" value={form.capacidad_max} onChange={e => setF('capacidad_max', e.target.value)} placeholder="50" />
            </div>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '3px' }}>Asistentes reales</label>
              <input style={inputStyle} type="number" value={form.asistentes_real} onChange={e => setF('asistentes_real', e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '3px' }}>Costo estimado</label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 1fr' : '1fr', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389', fontSize: '13px' }}>No hay eventos.</div>
          ) : filtered.map(ev => {
            ...
            return (
              <div key={ev.id} onClick={() => setSelected(selected?.id === ev.id ? null : ev)}
                style={{ background: 'white', border: `1.5px solid ${selected?.id === ev.id ? '#1B3B36' : '#E1DDD0'}`, borderRadius: '10px', padding: '12px 14px', cursor: 'pointer' }}>
        ...
        {selected && (
          <div style={{ background: '#FAF7EF', border: '1.5px solid #E1DDD0', borderRadius: '12px', padding: '14px' }}>
            ...
            {showAsistenteForm && (
              <div style={{ background: 'white', borderRadius: '8px', padding: '10px', marginBottom: '10px', border: '1px solid #E1DDD0' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <div>
                    <label style={{ fontSize: '10px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '2px' }}>Nombre contacto</label>
                  ...
                  <div>
                    <label style={{ fontSize: '10px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '2px' }}>Nº personas</label>
            ...
            {selectedAsistentes.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px', color: '#7E9389', fontSize: '12px' }}>Sin asistentes registrados.</div>
            ) : (
                      ...
                      {canEdit && (
                        <div style={{ display: 'flex', gap: '4px' }}>
                          <button onClick={() => marcarAsistio(a.id, !a.asistio)}
                            style={{ padding: '3px 7px', background: a.asistio ? '#dcfce7' : '#EAE6D8', color: a.asistio ? '#16a34a' : '#7E9389', border: 'none', borderRadius: '5px', fontSize: '10px', cursor: 'pointer', fontWeight: 600 }}>

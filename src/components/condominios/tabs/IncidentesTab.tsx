...
const TIPO_STYLE: Record<string, { bg: string; color: string; label: string; icon: string }> = {
  ...
  pelea:      { bg: '#f3e8ff', color: '#7c3aed', label: 'Pelea',      icon: '⚡' },
  otro:       { bg: '#f1f5f9', color: '#64748b', label: 'Otro',       icon: '📋' },
}
...
export function IncidentesTab({ incidentes, proyectoId, companyId, proyectoNombre = 'Condominio', canCreate, canEdit, onRefresh }: Props) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 0 }}>
      {/* LEFT */}
      <div style={{ width: '360px', flexShrink: 0, borderRight: '1.5px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '14px', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', marginBottom: '10px', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: '60px', background: abiertos > 0 ? '#fee2e2' : '#f0fdf4', borderRadius: '8px', padding: '6px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '10px', color: '#64748b', fontWeight: 600 }}>Abiertos</div>
            </div>
            <div style={{ flex: 1, minWidth: '60px', background: '#f1f5f9', borderRadius: '8px', padding: '6px', textAlign: 'center' }}>
              <div style={{ fontWeight: 800, fontSize: '18px', color: '#0f172a' }}>{incidentes.length}</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px', color: '#94a3b8', fontSize: '13px' }}>Sin incidentes</div>
          )}
          {filtered.map(i => {
            ...
            return (
              <div key={i.id} onClick={() => { setSelected(i); setShowForm(false) }}
                style={{ padding: '12px 14px', borderBottom: '1px solid #f1f5f9', cursor: 'pointer', background: selected?.id === i.id ? '#fff1f2' : 'white', borderLeft: selected?.id === i.id ? '3px solid #ef4444' : '3px solid transparent' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      <span style={{ fontSize: '16px' }}>{ts.icon}</span>
                      <span style={{ fontWeight: 700, fontSize: '13px', color: '#0f172a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{i.descripcion}</span>
                    </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        {showForm && (
          <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '12px' }}>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Reportado por</label>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Involucrados</label>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Seguimiento</label>
          ...
          return (
            <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '24px' }}>
              ...
              <div style={{ background: '#f8fafc', borderRadius: '8px', padding: '14px', marginBottom: '14px' }}>
                <div style={{ fontSize: '11px', fontWeight: 600, color: '#94a3b8', marginBottom: '6px' }}>Descripción</div>
                <p style={{ margin: 0, fontSize: '14px', color: '#0f172a', lineHeight: 1.6 }}>{selected.descripcion}</p>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '14px' }}>
                {[
                  ...
                  <div key={row.label} style={{ background: '#f8fafc', borderRadius: '8px', padding: '10px 12px' }}>
                    <div style={{ fontSize: '11px', fontWeight: 600, color: '#94a3b8', marginBottom: '2px' }}>{row.label}</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#374151' }}>{row.val}</div>
                  </div>
                ))}
              </div>
              ...
              {selected.seguimiento && (
                <div style={{ background: '#fefce8', borderRadius: '8px', padding: '12px', marginBottom: '16px', border: '1px solid #fde68a' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '13px', color: '#374151' }}>{selected.seguimiento}</p>
                </div>
              )}

              {/* Estado actions */}
              {canEdit && es.next && (
                <button onClick={() => avanzarEstado(selected)} style={{ padding: '9px 20px', background: '#0ea5e9', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '13px' }}>
                  → {es.nextLabel}
                </button>
              )}
            </div>
        ...
        {!showForm && !selected && (
          <div style={{ textAlign: 'center', padding: '60px', color: '#94a3b8' }}>
            ...
            <p style={{ fontWeight: 600, color: '#64748b' }}>Selecciona un incidente o registra uno nuevo</p>
          </div>
        )}
      </div>
    </div>

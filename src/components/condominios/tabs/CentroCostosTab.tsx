...
const CAT_COLOR: Record<string, string> = {
  mantenimiento: '#3b82f6', servicios: '#8b5cf6', administracion: '#f59e0b',
  seguridad: '#ef4444', limpieza: '#10b981', mejoras: '#6366f1', otro: '#94a3b8',
}
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#0f172a' }}>Centro de costos</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        ...
        <div style={{ background: '#f5f3ff', borderRadius: 10, padding: '12px 16px' }}>
          ...
          <div style={{ fontSize: 11, color: '#7c3aed', fontWeight: 600 }}>Ratio gasto/ingreso</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
          ...
          {porCategoria.length === 0 ? (
            <div style={{ color: '#9ca3af', fontSize: 12, textAlign: 'center', padding: '24px 0' }}>Sin gastos en {anio}</div>
          ) : porCategoria.map(([cat, monto]) => (
            <div key={cat} style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                <span style={{ textTransform: 'capitalize', color: '#374151' }}>{cat}</span>
                <span style={{ fontWeight: 600, color: CAT_COLOR[cat] ?? '#94a3b8' }}>
              ...
              <div style={{ background: '#f3f4f6', borderRadius: 4, height: 7 }}>
                <div style={{ height: '100%', background: CAT_COLOR[cat] ?? '#94a3b8', width: `${(monto / maxCat) * 100}%`, borderRadius: 4 }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 3, alignItems: 'flex-end', height: 120, marginBottom: 6 }}>
            {meses.map(m => (
              <div key={m.k} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                <div
                  style={{ width: '100%', background: '#dbeafe', borderRadius: '3px 3px 0 0', height: `${(m.monto / maxMes) * 100}%`, minHeight: m.monto > 0 ? 3 : 0 }}
          ...
          {gastoTotal > 0 && (
            <div style={{ padding: '8px 12px', background: '#f8fafc', borderRadius: 8, fontSize: 11, color: '#6b7280', textAlign: 'center' }}>
              Promedio mensual: <strong style={{ color: '#374151' }}>{moneda} {(gastoTotal / 12).toLocaleString('es', { minimumFractionDigits: 2 })}</strong>

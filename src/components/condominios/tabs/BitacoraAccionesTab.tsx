            ...
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                {['Fecha/Hora', 'Usuario', 'Acción', 'Módulo', 'Registro', ''].map(h => (
                  <th key={h} style={{ padding: '8px 12px', textAlign: 'left', color: '#6b7280', fontWeight: 600, fontSize: 11, borderBottom: '1px solid #e5e7eb', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {lista.map(b => {
                ...
                const cfg = ACCION_CFG[accionKey] ?? { label: b.accion, bg: '#f3f4f6', color: '#374151', icon: '•' }
                return (
                  <tr key={b.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '8px 12px', color: '#374151' }}>
                      <span style={{ padding: '2px 7px', borderRadius: 6, background: '#f3f4f6', fontSize: 11 }}>{b.modulo}</span>
                    </td>
                    <td style={{ padding: '8px 12px', color: '#374151', maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>

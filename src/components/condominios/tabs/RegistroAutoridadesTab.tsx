...
const TIPOS: { value: TipoAutoridad; label: string; icon: string; color: string }[] = [
  { value: 'policia',        label: 'Policía',        icon: '👮', color: '#3b82f6' },
  { value: 'bomberos',       label: 'Bomberos',       icon: '🚒', color: '#ef4444' },
  { value: 'salud',          label: 'Salud',          icon: '🏥', color: '#10b981' },
  { value: 'municipalidad',  label: 'Municipalidad',  icon: '🏛️', color: '#8b5cf6' },
  { value: 'electricidad',   label: 'Electricidad',   icon: '⚡', color: '#f59e0b' },
  { value: 'agua',           label: 'Agua',           icon: '💧', color: '#06b6d4' },
...
const RESULTADOS: { value: ResultadoAutoridad; label: string; color: string; bg: string }[] = [
  ...
  { value: 'recomendacion',     label: 'Recomendación',     color: '#6366f1', bg: '#eef2ff' },
        ...
        {lista.map(r => {
          ...
          return (
            <div key={r.id} onClick={() => { setSelected(r); setMostrarForm(false) }}
              style={{ padding: '10px 12px', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', background: selected?.id === r.id ? '#eef2ff' : '#fff', borderLeft: `3px solid ${r.requiere_seguimiento ? '#f59e0b' : tipo?.color}` }}>
              ...
              <div style={{ fontSize: 12, color: '#374151', marginTop: 2, lineHeight: 1.4 }}>{r.motivo.length > 60 ? r.motivo.slice(0, 60) + '…' : r.motivo}</div>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>{selected.fecha}{selected.hora_llegada && <span> · Llegada: {selected.hora_llegada.slice(0, 5)}</span>}{selected.hora_salida && <span> · Salida: {selected.hora_salida.slice(0, 5)}</span>}</div>
                </div>
                {selected.resultado && (
              ...
              {selected.observaciones && (
                <div style={{ background: '#f9fafb', borderRadius: 8, padding: '12px 16px', fontSize: 13, color: '#374151', lineHeight: 1.6 }}>
                  <strong style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>OBSERVACIONES</strong>
                  {selected.observaciones}
                </div>
              )}
            </div>

...
const ESTADO_CFG: Record<string, { color: string; bg: string }> = {
  ...
  activo:   { color: '#1B3B36', bg: '#EEF2EC' },
  aplicado: { color: '#9C5733', bg: '#FAF1EA' },
}
  ...
  function imprimir() {
    const filas = movimientos.map(m => `
      <tr>
        ...
        <td style="text-align:center"><span style="padding:2px 8px;border-radius:12px;background:${ESTADO_CFG[m.estado]?.bg ?? '#EAE6D8'};color:${ESTADO_CFG[m.estado]?.color ?? '#3E5A4C'};font-size:11px">${m.estado}</span></td>
      </tr>`).join('')
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ fontWeight: 700, fontSize: 15, color: '#15291F' }}>Estado de Cuenta por Residente</div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => exportarPDFEstadoCuenta(movimientos, unidad?.nombre ?? unidadId, anio, moneda, proyectoNombre)}
            style={{ padding: '6px 16px', background: '#577B69', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>
            📄 Descargar PDF
          </button>
          <button onClick={imprimir}
            style={{ padding: '6px 16px', background: '#1B3B36', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>
            🖨️ Imprimir
          </button>
        </div>
      </div>
            ...
            <thead>
              <tr style={{ background: '#FAF7EF' }}>
                {['Fecha', 'Descripción', 'Cargo', 'Abono', 'Saldo acum.', 'Estado'].map(h => (
                  <th key={h} style={{ padding: '9px 12px', textAlign: h === 'Fecha' || h === 'Descripción' ? 'left' : 'right', color: '#7E9389', fontWeight: 600, fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {movimientos.reduce<{ rows: JSX.Element[]; saldoAcum: number }>((acc, m, i) => {
                ...
                const sCfg = ESTADO_CFG[m.estado] ?? { color: '#3E5A4C', bg: '#EAE6D8' }
                acc.rows.push(
                  <tr key={i} style={{ borderTop: i > 0 ? '1px solid #EAE6D8' : undefined }}>
                    <td style={{ padding: '9px 12px', color: '#7E9389' }}>{m.fecha}</td>
                    <td style={{ padding: '9px 12px', color: '#15291F' }}>{m.descripcion}</td>
                    <td style={{ padding: '9px 12px', textAlign: 'right', color: m.cargo > 0 ? '#ef4444' : '#7E9389' }}>{m.cargo > 0 ? `${moneda} ${m.cargo.toFixed(2)}` : '—'}</td>
                    <td style={{ padding: '9px 12px', textAlign: 'right', color: m.abono > 0 ? '#16a34a' : '#7E9389' }}>{m.abono > 0 ? `${moneda} ${m.abono.toFixed(2)}` : '—'}</td>

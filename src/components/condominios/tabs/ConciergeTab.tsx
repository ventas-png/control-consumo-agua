    ...
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'Facturado/mes',   value: costoMes > 0 ? `${moneda} ${costoMes.toFixed(0)}` : '—',             icon: '💰', color: '#8b5cf6' },
        ].map(k => (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Concierge Digital</h2>
        {canCreate && !showForm && (
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {Object.entries(byTipo).map(([tipo, items]) => {
            ...
            return (
              <div key={tipo}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#64748b', marginBottom: '10px' }}>{tc.icon} {tc.label} ({items.length})</div>
                    ...
                    return (
                      <div key={s.id} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '10px', padding: '14px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                          <div style={{ flex: 1, marginRight: '8px', fontSize: '13px', fontWeight: 600, color: '#0f172a' }}>{s.descripcion}</div>
                        ...
                        <div style={{ fontSize: '12px', color: '#64748b', display: 'flex', flexDirection: 'column', gap: '3px', marginBottom: '10px' }}>
                          ...
                          {s.costo && <div style={{ fontWeight: 700, color: '#0f172a' }}>💰 {moneda} {s.costo.toFixed(2)}</div>}
                        </div>
                        {canEdit && (
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                            {s.estado === 'pendiente' && <button onClick={() => handleEstado(s.id, 'en_proceso')} style={{ flex: 1, padding: '4px 8px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>Iniciar</button>}

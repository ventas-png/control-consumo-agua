...
export default function AnalisisVisitantesTab({ visitantes, unidades }: Props) {
  ...
  function heatColor(val: number, max: number): string {
    if (val === 0) return 'var(--at-chip)'
    const p = val / max
    if (p > 0.75) return 'var(--at-primary-hover)'
    if (p > 0.5)  return 'var(--at-primary)'
    if (p > 0.25) return 'var(--at-accent-2)'
    return 'var(--at-primary-soft-2)'
  }

  return (
    <div style={{ padding: 16 }}>
      <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--at-ink)', marginBottom: 2 }}>Análisis de Visitantes</div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--at-ink)', marginBottom: 10 }}>Horas de mayor afluencia</div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, fontSize: 9, color: 'var(--at-ink-3)' }}>
            ...
            {/* Días de semana */}
            <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--at-ink)', marginBottom: 10 }}>Visitas por día de semana</div>
              <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 80 }}>
                {DIAS.map((d, i) => (
                  <div key={d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--at-ink-3)', textAlign: 'center' }}>{d}</div>
                  </div>
                ))}
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
            {/* Tendencia mensual */}
            <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--at-ink)', marginBottom: 10 }}>Tendencia últimos 6 meses</div>
              {stats.tendenciaMes.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--at-ink-3)' }}>Sin datos suficientes</div>
              ) : (
                <>
                  <div style={{ display: 'flex', gap: 4, alignItems: 'flex-end', height: 70 }}>
                    {stats.tendenciaMes.map(m => (
                      <div key={m.mes} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <div style={{ width: '100%', height: `${(m.cnt / maxTend) * 60}px`, background: 'var(--at-primary)', opacity: 0.75, borderRadius: '3px 3px 0 0', minHeight: 2 }} />
                      </div>
                    ))}
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                    {stats.tendenciaMes.map(m => (
                      <div key={m.mes} style={{ flex: 1, textAlign: 'center' }}>
                        <div style={{ fontSize: 9, color: 'var(--at-ink-3)' }}>{labelMes(m.mes)}</div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--at-primary)' }}>{m.cnt}</div>
                      </div>
                    ))}
                  </div>
                </>
            ...
            {/* Top unidades */}
            <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--at-ink)', marginBottom: 10 }}>Unidades más visitadas</div>
              {stats.topUnidades.slice(0, 6).map((u, i) => (
                <div key={u.uid} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--at-primary)' }}>{u.cnt}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--at-ink)', marginBottom: 10 }}>Visitantes frecuentes</div>
              {stats.visitantesFrecuentes.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--at-ink-3)' }}>Sin visitantes recurrentes</div>
              ) : (
                stats.visitantesFrecuentes.slice(0, 6).map(([nombre, cnt], i) => (
                  <div key={nombre} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 12, color: 'var(--at-ink-2)', textTransform: 'capitalize', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {i === 0 ? '⭐ ' : ''}{nombre}
                    </span>
                    <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--at-accent-hover)', flexShrink: 0 }}>{cnt} visitas</span>
                  </div>
                ))
              )}
            </div>

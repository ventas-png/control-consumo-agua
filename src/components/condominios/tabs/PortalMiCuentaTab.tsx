...
export function PortalMiCuentaTab({ cuotas, moneda, unidadNombre }: Props) {
  ...
  return (
    <div>
      <h3 style={{ margin: '0 0 18px', fontSize: '17px', fontWeight: 700, color: '#0f172a' }}>Mi cuenta — {unidadNombre}</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        <div style={{ background: totalDeuda > 0 ? '#fef2f2' : '#f0fdf4', borderRadius: '14px', padding: '16px', border: `1.5px solid ${totalDeuda > 0 ? '#fecaca' : '#86efac'}` }}>
          <div style={{ fontSize: '11.5px', fontWeight: 600, color: '#64748b', marginBottom: '4px' }}>SALDO PENDIENTE</div>
        ...
        <div style={{ background: '#f0fdf4', borderRadius: '14px', padding: '16px', border: '1.5px solid #86efac' }}>
          <div style={{ fontSize: '11.5px', fontWeight: 600, color: '#64748b', marginBottom: '4px' }}>TOTAL PAGADO</div>
          <div style={{ fontSize: '22px', fontWeight: 800, color: '#16a34a' }}>{moneda} {totalPagado.toFixed(2)}</div>
          <div style={{ fontSize: '11.5px', color: '#64748b', marginTop: '3px' }}>{pagadas.length} pago{pagadas.length !== 1 ? 's' : ''}</div>
        </div>
        <div style={{ background: '#eff6ff', borderRadius: '14px', padding: '16px', border: '1.5px solid #bfdbfe' }}>
          <div style={{ fontSize: '11.5px', fontWeight: 600, color: '#64748b', marginBottom: '4px' }}>CUOTAS PENDIENTES</div>
          <div style={{ fontSize: '22px', fontWeight: 800, color: '#2563eb' }}>{pendientes.length}</div>
          <div style={{ fontSize: '11.5px', color: '#64748b', marginTop: '3px' }}>de {cuotas.length} en total</div>
        </div>
      </div>
      ...
      {pendientes.length > 0 && (
        <div style={{ marginBottom: '24px' }}>
          <h4 style={{ margin: '0 0 10px', fontSize: '14px', fontWeight: 700, color: '#374151' }}>Cuotas por pagar</h4>
              ...
              return (
                <div key={c.id} style={{ background: 'white', border: `1.5px solid ${esVencida ? '#fecaca' : '#fde68a'}`, borderRadius: '12px', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '14px', color: '#0f172a' }}>{c.concepto.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} — {c.periodo}</div>
                    {c.fecha_vencimiento && (
                      <div style={{ fontSize: '12.5px', color: esVencida ? '#dc2626' : '#64748b', marginTop: '2px' }}>
                    ...
                    {c.notas && <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '2px' }}>{c.notas}</div>}
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontWeight: 800, fontSize: '16px', color: '#0f172a' }}>{moneda} {c.monto.toFixed(2)}</div>
      ...
      {pagadas.length > 0 && (
        <div>
          <h4 style={{ margin: '0 0 10px', fontSize: '14px', fontWeight: 700, color: '#374151' }}>Historial de pagos</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {pagadas.sort((a, b) => (b.fecha_pago ?? b.created_at) < (a.fecha_pago ?? a.created_at) ? -1 : 1).map(c => (
              <div key={c.id} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '18px' }}>✅</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: '13.5px', color: '#374151' }}>{c.concepto.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} — {c.periodo}</div>
                  {c.fecha_pago && <div style={{ fontSize: '12px', color: '#64748b' }}>Pagado el {new Date(c.fecha_pago + 'T12:00:00').toLocaleDateString('es', { day: '2-digit', month: 'short', year: 'numeric' })}{c.metodo_pago ? ` · ${c.metodo_pago}` : ''}</div>}
                </div>
      ...
      {cuotas.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>
          ...
          <p style={{ fontWeight: 600, color: '#64748b' }}>No hay cuotas registradas para esta unidad</p>
        </div>
      )}
    </div>
  )

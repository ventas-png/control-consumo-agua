...
const TIPO_LABELS: Record<TipoActa, { label: string; icon: string; color: string }> = {
  ordinaria:    { label: 'Ordinaria',    icon: '📋', color: '#0ea5e9' },
  extraordinaria:{ label: 'Extraordinaria', icon: '⚡', color: '#f59e0b' },
  junta:        { label: 'Junta Directiva', icon: '👑', color: '#8b5cf6' },
...
export function ActasTab({ actas, proyectoId, companyId, canCreate, canEdit, onRefresh }: Props) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', padding: '7px 10px', border: '1.5px solid #e2e8f0', borderRadius: '7px', fontSize: '13px', color: '#1e293b', background: '#f8fafc', boxSizing: 'border-box' }

  return (
    <div style={{ padding: '20px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Actas de Reunión</h2>
        {canCreate && !showForm && (
      ...
      {showForm && (
        <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '14px' }}>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Quórum asistente</label>
              <input style={inputStyle} type="number" min="0" value={form.quorum} onChange={e => setF('quorum', e.target.value)} placeholder="0" />
            </div>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Quórum requerido</label>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Redactada por</label>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingTop: '18px' }}>
              ...
              <label htmlFor="aprobada_chk" style={{ fontSize: '12px', color: '#64748b', cursor: 'pointer' }}>Acta aprobada</label>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              <label style={{ fontSize: '12px', fontWeight: 700, color: '#0f172a' }}>Asistentes</label>
              <button onClick={addAsistente} style={{ padding: '2px 8px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '5px', fontSize: '11px', cursor: 'pointer' }}>+ Agregar</button>
            </div>
            {asistentes.map((a, i) => (
          ...
          <div style={{ marginBottom: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              <label style={{ fontSize: '12px', fontWeight: 700, color: '#0f172a' }}>Orden del Día</label>
              <button onClick={addPunto} style={{ padding: '2px 8px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '5px', fontSize: '11px', cursor: 'pointer' }}>+ Punto</button>
            </div>
            {puntos.map((p, i) => (
              <div key={i} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '8px', padding: '10px', marginBottom: '6px' }}>
          ...
          <div style={{ marginBottom: '10px' }}>
            <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Acuerdos generales</label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: viewed ? '1fr 380px' : '1fr', gap: '16px' }}>
          ...
          {actas.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {actas.map(a => {
                ...
                return (
                  <div key={a.id} style={{ background: viewId === a.id ? '#eff6ff' : 'white', border: `1.5px solid ${viewId === a.id ? '#bfdbfe' : '#e2e8f0'}`, borderLeft: `4px solid ${tl.color}`, borderRadius: '8px', padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => setViewId(viewId === a.id ? null : a.id)}>
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                          <span style={{ fontWeight: 700, fontSize: '13px', color: '#0f172a' }}>{tl.icon} {a.titulo}</span>
                      ...
                      {canEdit && (
                        <div style={{ display: 'flex', gap: '4px', flexShrink: 0, marginLeft: '8px' }}>
                          <button onClick={() => handlePrint(a)} style={{ padding: '3px 7px', background: '#f1f5f9', border: 'none', borderRadius: '5px', fontSize: '11px', cursor: 'pointer' }}>🖨️</button>
                          ...
                          <button onClick={() => startEdit(a)} style={{ padding: '3px 7px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '5px', fontSize: '11px', cursor: 'pointer' }}>✏️</button>
        ...
        {/* Detail panel */}
        {viewed && (
          <div style={{ border: '1.5px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden', alignSelf: 'start', maxHeight: '70vh', overflowY: 'auto' }}>
            <div style={{ padding: '10px 14px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0 }}>
              <span style={{ fontSize: '12px', fontWeight: 700, color: '#64748b' }}>Detalle del acta</span>
              <button onClick={() => setViewId(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: '16px' }}>×</button>
            </div>
            <div style={{ padding: '14px', fontSize: '12px', lineHeight: '1.6' }}>
              {(viewed.asistentes as AsistenteFila[]).length > 0 && (
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '4px' }}>Asistentes ({(viewed.asistentes as AsistenteFila[]).length})</div>
                  {(viewed.asistentes as AsistenteFila[]).map((a, i) => (
                    <div key={i} style={{ padding: '3px 0', color: '#64748b' }}>• {a.nombre}{a.unidad ? ` — ${a.unidad}` : ''}{a.rol ? ` (${a.rol})` : ''}</div>
                  ))}
                </div>
              )}
              {(viewed.orden_del_dia as PuntoAgenda[]).length > 0 && (
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '6px' }}>Orden del Día</div>
                  {(viewed.orden_del_dia as PuntoAgenda[]).map((p, i) => (
                    <div key={i} style={{ marginBottom: '8px', paddingLeft: '8px', borderLeft: '3px solid #e2e8f0' }}>
                      <div style={{ fontWeight: 600 }}>{i+1}. {p.punto}</div>
                      {p.descripcion && <div style={{ color: '#64748b', marginTop: '2px' }}>{p.descripcion}</div>}
              ...
              {viewed.acuerdos && <div style={{ marginBottom: '10px' }}><div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '3px' }}>Acuerdos generales</div><div style={{ color: '#64748b', whiteSpace: 'pre-wrap' }}>{viewed.acuerdos}</div></div>}
              {viewed.observaciones && <div><div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '3px' }}>Observaciones</div><div style={{ color: '#64748b', whiteSpace: 'pre-wrap' }}>{viewed.observaciones}</div></div>}
            </div>
          </div>
        )}
      </div>

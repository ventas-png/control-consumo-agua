...
export default function EstacionamientoVisitaTab({
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13 }
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div>
        ...
        {historial.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#9ca3af', padding: '24px 0', fontSize: 13 }}>Sin registros de salidas para esta fecha</div>
        ) : (
              ...
              {historial.map(r => {
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '8px 10px', color: '#6366f1' }}>{r.hora_salida ? duracion(r.hora_entrada, r.hora_salida) : '—'}</td>
                  </tr>
                )
              })}
            </tbody>

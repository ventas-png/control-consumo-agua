            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {porUnidad.map((u, i) => (
                <div key={u.unidad_nombre} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: 'var(--at-surface-2)', borderRadius: 8 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: i < 3 ? '#fef2f2' : 'var(--at-chip)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: i < 3 ? '#ef4444' : 'var(--at-ink-3)', flexShrink: 0 }}>
                    {i + 1}
                  </div>

...
const TIPO_CONFIG: Record<TipoAnuncio, { label: string; icon: string; bg: string; color: string; border: string }> = {
  aviso:         { label: 'Aviso',         icon: '📢', bg: '#EEF2EC', color: '#1B3B36', border: '#C2D2CA' },
...
export function PortalAnunciosTab({ anuncios }: Props) {
  ...
  return (
    <div>
      <h3 style={{ margin: '0 0 18px', fontSize: '17px', fontWeight: 700, color: '#15291F' }}>Comunicados del condominio</h3>
        ...
        <div style={{ background: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: '12px', padding: '14px 16px', marginBottom: '18px' }}>
          ...
          {urgentes.map(a => (
            <div key={a.id} style={{ marginBottom: '6px' }}>
              <div style={{ fontWeight: 700, fontSize: '14px', color: '#15291F' }}>{a.titulo}</div>
              <div style={{ fontSize: '13px', color: '#3E5A4C' }}>{a.contenido}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {activos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389' }}>
          ...
          <p style={{ fontWeight: 600, color: '#7E9389' }}>Sin comunicados recientes</p>
        </div>
      ) : (
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '4px' }}>
                      <span style={{ fontWeight: 700, fontSize: '14.5px', color: '#15291F' }}>{a.titulo}</span>
                      <span style={{ padding: '2px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 700, background: tc.bg, color: tc.color }}>{tc.label}</span>
                    </div>
                    <p style={{ margin: '0 0 6px', fontSize: '13.5px', color: '#3E5A4C', lineHeight: 1.5 }}>{a.contenido}</p>
                    <div style={{ fontSize: '11.5px', color: '#7E9389', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>

...
export function DirectorioTab({ personal, contactosEmergencia, proyectoId }: Props) {
  ...
  const tabStyle = (t: DirectorioTab): CSSProperties => ({
    ...
    cursor: 'pointer', background: activeTab === t ? '#1B3B36' : 'transparent', color: activeTab === t ? 'white' : '#7E9389',
  })
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      {/* Sub-tabs */}
      <div style={{ display: 'flex', gap: '4px', marginBottom: '16px', background: '#EAE6D8', borderRadius: '10px', padding: '4px', width: 'fit-content' }}>
      ...
      {activeTab === 'residentes' && (
        loading ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389' }}>Cargando…</div>
        ) : filteredResidentes.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389', fontSize: '13px' }}>No se encontraron residentes.</div>
        ) : (
            ...
            {filteredResidentes.map((r, i) => (
              <div key={i} style={cardStyle}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: '13px', color: '#15291F' }}>{r.unidad_nombre}</div>
                  {r.nombre ? (
                    <div style={{ fontSize: '12px', color: '#7E9389', marginTop: '2px' }}>{r.nombre}</div>
                  ) : (
                    <div style={{ fontSize: '12px', color: '#C7C2B0', fontStyle: 'italic' }}>Sin residente asignado</div>
                  )}
                  {r.telefono && (
                    <a href={`tel:${r.telefono}`} style={{ fontSize: '12px', color: '#1B3B36', display: 'block', marginTop: '2px', textDecoration: 'none' }}>📞 {r.telefono}</a>
                  )}
                  {r.email && (
                    <a href={`mailto:${r.email}`} style={{ fontSize: '11px', color: '#7E9389', display: 'block', textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>✉️ {r.email}</a>
                  )}
                </div>
              </div>
            ))}
      ...
      {activeTab === 'personal' && (
        filteredPersonal.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389', fontSize: '13px' }}>No se encontró personal.</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '10px' }}>
            {filteredPersonal.map(p => (
              <div key={p.id} style={cardStyle}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: '13px', color: '#15291F' }}>{p.nombre}</div>
                  <div style={{ fontSize: '12px', color: '#7E9389', textTransform: 'capitalize' }}>{p.cargo}</div>
                  {p.telefono && (
                    <a href={`tel:${p.telefono}`} style={{ fontSize: '12px', color: '#1B3B36', display: 'block', marginTop: '2px', textDecoration: 'none' }}>📞 {p.telefono}</a>
                  )}
                  {p.email && (
                    <a href={`mailto:${p.email}`} style={{ fontSize: '11px', color: '#7E9389', display: 'block', textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>✉️ {p.email}</a>
                  )}
                  <div style={{ marginTop: '4px' }}>
                    <span style={{ fontSize: '10px', fontWeight: 600, padding: '2px 6px', borderRadius: '20px',
                      background: p.estado === 'activo' ? '#dcfce7' : '#EAE6D8',
                      color: p.estado === 'activo' ? '#16a34a' : '#7E9389' }}>
                      {p.estado}
                    </span>
                  </div>
                </div>
      ...
      {activeTab === 'emergencias' && (
        filteredEmergencias.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389', fontSize: '13px' }}>No se encontraron contactos de emergencia.</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '10px' }}>
            {filteredEmergencias.map(c => (
              <div key={c.id} style={{ ...cardStyle, borderLeft: '4px solid #ef4444' }}>
                <div style={{ fontSize: '24px' }}>🚨</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: '13px', color: '#15291F' }}>{c.nombre}</div>
                  <div style={{ fontSize: '12px', color: '#7E9389', textTransform: 'capitalize' }}>{c.tipo}</div>
                  {c.telefono && (
                  ...
                  {c.telefono_alternativo && <div style={{ fontSize: '11px', color: '#7E9389' }}>Alt: {c.telefono_alternativo}</div>}

...
const ESTADO_STYLE: Record<string, { bg: string; color: string; label: string }> = {
  ...
  en_proceso:  { bg: '#e0f2fe', color: '#0369a1',  label: 'En proceso' },
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: '0 0 2px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Seguimiento de Acuerdos</h2>
          <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8' }}>Control de compromisos derivados de actas y reuniones</p>
        </div>
        {canCreate && !showForm && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px', marginBottom: '16px' }}>
        {[
          ...
          { label: 'En proceso',  value: String(enProceso),  color: '#0ea5e9' },
      ...
      {showForm && (
        <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px' }}>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Acta de origen</label>
            ...
            <div style={{ gridColumn: 'span 2' }}>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Descripción / Notas de seguimiento</label>
      ...
      {/* List */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontSize: '13px' }}>No hay acuerdos.</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.map(a => {
            ...
            return (
              <div key={a.id} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '10px', padding: '12px 14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#64748b', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                      ...
                      {a.fecha_limite && (
                        <span style={{ color: diasRestantes !== null && diasRestantes < 0 ? '#ef4444' : diasRestantes !== null && diasRestantes <= 7 ? '#f59e0b' : '#64748b', fontWeight: diasRestantes !== null && diasRestantes <= 7 ? 700 : 400 }}>
                          📅 {a.fecha_limite}{diasRestantes !== null ? ` (${diasRestantes >= 0 ? `${diasRestantes}d` : 'vencido'})` : ''}
                        </span>
                      )}
                    </div>
                    {a.notas_seguimiento && (
                      <div style={{ fontSize: '12px', color: '#374151', marginTop: '4px', fontStyle: 'italic' }}>{a.notas_seguimiento}</div>
                    )}
                  </div>
                  {canEdit && (
                    <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
                      {a.estado === 'pendiente' && (
                        <button onClick={() => cambiarEstado(a.id, 'en_proceso')}
                          style={{ padding: '3px 8px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '5px', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                          Iniciar
                        </button>
                      )}

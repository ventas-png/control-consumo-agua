  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: 'var(--at-ink)' }}>Llaves y Accesos</h2>
        {canCreate && !showForm && (
          <button onClick={() => setShowForm(true)} style={{ padding: '8px 16px', background: 'var(--at-primary)', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>+ Registrar Llave</button>
        )}
      </div>
        ...
        <div style={{ background: 'var(--at-surface-2)', border: '1.5px solid var(--at-line)', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingTop: '20px' }}>
              ...
              <label htmlFor="dep_pagado" style={{ fontSize: '12px', fontWeight: 600, color: 'var(--at-ink-3)', cursor: 'pointer' }}>Depósito pagado</label>
            </div>
              ...
              <div key={l.id} style={{ background: 'var(--at-surface)', border: `1.5px solid ${l.estado === 'perdida' ? 'var(--at-danger-border)' : 'var(--at-line)'}`, borderRadius: '10px', padding: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--at-ink-3)' }}>{tc.label}{l.cantidad > 1 ? ` · ${l.cantidad} copias` : ''}</div>
                  </div>

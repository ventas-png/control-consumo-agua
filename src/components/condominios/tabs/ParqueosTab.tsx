...
const TIPO_COLORS: Record<TipoParqueo, { bg: string; color: string }> = {
  ...
  discapacitado: { bg: '#f5f3ff', color: '#7c3aed' },
}
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>Parqueos</h2>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar por número, placa, unidad..."
          style={{ flex: 1, minWidth: '200px', padding: '8px 12px', border: '1.5px solid #e2e8f0', borderRadius: '8px', fontSize: '13.5px', background: '#f8fafc' }} />
        {(['todos', 'asignado', 'visita', 'discapacitado'] as const).map(t => (
          <button key={t} onClick={() => setFiltroTipo(t)}
            style={{ padding: '8px 14px', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: '1.5px solid', borderColor: filtroTipo === t ? '#0ea5e9' : '#e2e8f0', background: filtroTipo === t ? '#eff6ff' : 'white', color: filtroTipo === t ? '#0ea5e9' : '#64748b' }}>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Unidad asignada</label>
      ...
      {filtrados.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', color: '#94a3b8' }}>
          ...
          <p style={{ fontWeight: 600, color: '#64748b' }}>No hay parqueos registrados</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '12px' }}>
          {filtrados.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: 'white', border: `1.5px solid ${tieneUnidad ? '#bfdbfe' : '#e2e8f0'}`, borderRadius: '14px', padding: '16px', opacity: p.activo ? 1 : 0.55 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                  <div style={{ fontSize: '22px', fontWeight: 800, color: '#0f172a' }}>{p.numero}</div>
                ...
                {p.unidad_nombre && <div style={{ fontSize: '12.5px', color: '#2563eb', fontWeight: 600, marginBottom: '4px' }}>📍 {p.unidad_nombre}</div>}
                {p.placa_vehiculo && <div style={{ fontSize: '12.5px', color: '#374151', marginBottom: '2px' }}>🚗 {p.placa_vehiculo}{p.marca_vehiculo ? ` · ${p.marca_vehiculo}` : ''}{p.color_vehiculo ? ` · ${p.color_vehiculo}` : ''}</div>}
                {!tieneUnidad && <div style={{ fontSize: '12px', color: '#16a34a', fontWeight: 600 }}>Disponible</div>}
                {canEdit && (
                  <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
                    <button onClick={() => startEdit(p)} style={{ flex: 1, padding: '6px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '7px', cursor: 'pointer', fontSize: '12px', color: '#374151' }}>✏️ Editar</button>
                    <button onClick={() => toggleActivo(p)} style={{ padding: '6px 10px', background: p.activo ? '#fef3c7' : '#f0fdf4', border: '1px solid #e2e8f0', borderRadius: '7px', cursor: 'pointer', fontSize: '12px' }}>{p.activo ? '⏸' : '▶'}</button>

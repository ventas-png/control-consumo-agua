...
const ESTADO_CFG: Record<EstadoA, { label: string; color: string; bg: string }> = {
  ...
  finalizada: { label: 'Finalizada', color: '#64748b', bg: '#f1f5f9' },
...
export default function AsambleaDigitalTab({ asambleas, unidades, proyectoId, companyId, userId, canCreate, canEdit, onRefresh }: Props) {
  ...
  async function guardar() {
    if (!form.titulo.trim() || !form.fecha_hora) {
      return Swal.fire({ icon: 'warning', title: 'Faltan datos', text: 'Título y fecha son obligatorios.', confirmButtonColor: '#2563eb' })
    }
    setSaving(true)
    const payload = {
  ...
  function imprimirActa(a: AsambleaDigital) {
    ...
    h1{font-size:22px;color:#0f172a}h2{font-size:14px;color:#64748b;font-weight:normal}
    .meta{background:#f8fafc;padding:12px;border-radius:8px;margin:16px 0;font-size:12px}
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {(['lista', 'acta'] as const).map(t => (
          <button key={t} onClick={() => setSubTab(t)}
            style={{ padding: '5px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: subTab === t ? 700 : 400, background: subTab === t ? '#2563eb' : '#f1f5f9', color: subTab === t ? '#fff' : '#374151', fontSize: 13 }}>
      ...
      {subTab === 'acta' && actaAsamblea ? (
        <div>
          <button onClick={() => setSubTab('lista')} style={{ marginBottom: 12, padding: '4px 12px', border: '1px solid #d1d5db', borderRadius: 8, background: '#fff', cursor: 'pointer', fontSize: 12 }}>← Volver</button>
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{actaAsamblea.titulo}</div>
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>{new Date(actaAsamblea.fecha_hora).toLocaleString('es')}</div>
          <textarea
            value={actaTexto}
            ...
            style={{ width: '100%', padding: 12, borderRadius: 8, border: '1px solid #d1d5db', fontSize: 13, fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' }}
          />
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            ...
            <button onClick={() => imprimirActa({ ...actaAsamblea, acta_url: actaTexto })}
              style={{ padding: '8px 16px', background: '#f1f5f9', color: '#374151', border: '1px solid #d1d5db', borderRadius: 8, cursor: 'pointer' }}>
              🖨️ Imprimir
            </button>
          </div>
        </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a' }}>Asambleas Digitales</div>
            {canCreate && !showForm && (
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ padding: '8px 14px', background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 10, minWidth: 90, textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#0f172a' }}>{unidades.length}</div>
          ...
          {showForm && (
            <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16, marginBottom: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12, color: '#0f172a' }}>{editId ? 'Editar asamblea' : 'Nueva asamblea'}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ fontSize: 12, color: '#64748b' }}>Título *</label>
                ...
                <div>
                  <label style={{ fontSize: 12, color: '#64748b' }}>Fecha y hora *</label>
                  <input type="datetime-local" value={form.fecha_hora} onChange={e => setForm(f => ({ ...f, fecha_hora: e.target.value }))}
                    style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 13, boxSizing: 'border-box', marginTop: 3 }} />
                </div>
                <div>
                  <label style={{ fontSize: 12, color: '#64748b' }}>Modalidad</label>
                ...
                <div>
                  <label style={{ fontSize: 12, color: '#64748b' }}>Quórum requerido (%)</label>
                ...
                {(form.modalidad === 'virtual' || form.modalidad === 'hibrida') && (
                  <div style={{ gridColumn: '1 / -1' }}>
                    <label style={{ fontSize: 12, color: '#64748b' }}>Link de reunión</label>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ fontSize: 12, color: '#64748b' }}>Descripción / Agenda</label>
          ...
          {filtradas.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {filtradas.map(a => {
                ...
                return (
                  <div key={a.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
                    <div onClick={() => setExpandida(exp ? null : a.id)}
                      style={{ padding: '12px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 13, color: '#0f172a' }}>{a.titulo}</div>
                    ...
                    {exp && (
                      <div style={{ padding: '0 16px 16px', borderTop: '1px solid #f1f5f9' }}>
                        {a.descripcion && (
                          <div style={{ fontSize: 12, color: '#374151', marginTop: 10, whiteSpace: 'pre-wrap' }}>{a.descripcion}</div>
                        )}
                        {a.link_reunion && (
                          <div style={{ marginTop: 8, fontSize: 12 }}>
                            🔗 <a href={a.link_reunion} target="_blank" rel="noreferrer" style={{ color: '#2563eb' }}>{a.link_reunion}</a>
                          </div>
                        )}
                        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                          ...
                          {canEdit && a.estado === 'en_curso' && (
                            <button onClick={() => cambiarEstado(a, 'finalizada')}
                              style={{ padding: '6px 14px', background: '#f1f5f9', color: '#64748b', border: '1px solid #64748b', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                              ✅ Finalizar
                            </button>
                          )}
                          {(a.estado === 'en_curso' || a.estado === 'finalizada') && (
                            <button onClick={() => abrirActa(a)}
                              style={{ padding: '6px 14px', background: '#f5f3ff', color: '#7c3aed', border: '1px solid #7c3aed', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                              📝 {a.acta_url ? 'Ver/editar acta' : 'Registrar acta'}
                            </button>
                          )}
                          {a.acta_url && (
                            <button onClick={() => imprimirActa(a)}
                              style={{ padding: '6px 12px', background: '#f8fafc', color: '#374151', border: '1px solid #d1d5db', borderRadius: 8, cursor: 'pointer', fontSize: 12 }}>
                              🖨️ Imprimir
                            </button>
                          )}
                          {canEdit && a.estado !== 'en_curso' && (

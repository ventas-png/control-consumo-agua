  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          { label: 'Reglas activas', val: activas, bg: '#dbeafe', color: '#1d4ed8' },
          { label: 'Reglas inactivas', val: reglas.length - activas, bg: '#f3f4f6', color: '#6b7280' },
          { label: 'Total reglas', val: reglas.length, bg: '#f9fafb', color: '#374151' },
        ].map(k => (
      ...
      {reglas.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {reglas.map(r => (
            <div key={r.id} style={{ background: '#fff', border: `1px solid ${r.activa ? '#e5e7eb' : '#f3f4f6'}`, borderRadius: 10, padding: '12px 16px', opacity: r.activa ? 1 : 0.6 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span style={{ fontWeight: 700, fontSize: 14 }}>{r.nombre}</span>
                    <span style={{ padding: '1px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700, background: r.activa ? '#dcfce7' : '#f3f4f6', color: r.activa ? '#16a34a' : '#9ca3af' }}>
                  ...
                  {r.notas && <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 4 }}>{r.notas}</div>}
                </div>
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  {canEdit && (
                    <button onClick={() => abrirEditar(r)}
                      style={{ padding: '5px 10px', background: '#eff6ff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 11, color: '#2563eb' }}>
                      ✏️
                    </button>
                  )}
                  {canEdit && (

...
const ESTADO_INF_CFG: Record<EstadoInfraccion, { label: string; color: string; bg: string }> = {
  ...
  notificada:   { label: 'Notificada',    color: '#1B3B36', bg: '#EEF2EC' },
  en_descargo:  { label: 'En descargo',   color: '#9C5733', bg: '#FAF1EA' },
  resuelta:     { label: 'Resuelta',      color: '#16a34a', bg: '#dcfce7' },
  anulada:      { label: 'Anulada',       color: '#7E9389', bg: '#FAF7EF' },
}

const ESTADO_SUG_CFG: Record<EstadoSugerencia, { label: string; color: string; bg: string }> = {
  pendiente:   { label: 'Pendiente',   color: '#d97706', bg: '#fef3c7' },
  en_revision: { label: 'En revisión', color: '#1B3B36', bg: '#EEF2EC' },
  respondida:  { label: 'Respondida',  color: '#16a34a', bg: '#dcfce7' },
  archivada:   { label: 'Archivada',   color: '#7E9389', bg: '#FAF7EF' },
}
...
export default function GestionConflictosTab({ infracciones, sugerencias, unidades: _unidades, canEdit, onRefresh }: Props) {
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ fontWeight: 700, fontSize: 15, color: '#15291F', marginBottom: 2 }}>Gestión de Conflictos</div>
              ...
              <div key={inf.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#3E5A4C', fontWeight: 600 }}>{inf.tipo.replace('_', ' ')}</span>
                    {inf.unidad_nombre && <span style={{ fontSize: 10, color: '#7E9389' }}>· {inf.unidad_nombre}</span>}
                  </div>
                  <div style={{ fontSize: 12, color: '#15291F', marginBottom: 2 }}>{inf.descripcion}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color, background: cfg.bg,
                        padding: '2px 8px', borderRadius: 10, border: `1px solid ${cfg.color}33` }}>{cfg.label}</span>
                      <span style={{ fontSize: 11, color: '#3E5A4C', fontWeight: 600 }}>{sug.categoria}</span>
                      {sug.unidad_nombre && <span style={{ fontSize: 10, color: '#7E9389' }}>· {sug.unidad_nombre}</span>}
                      {sug.anonima && <span style={{ fontSize: 9, color: '#7E9389', fontStyle: 'italic' }}>anónima</span>}
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#15291F', marginBottom: 2 }}>{sug.titulo}</div>
                    <div style={{ fontSize: 11, color: '#3E5A4C', marginBottom: sug.respuesta ? 6 : 0 }}>{sug.descripcion}</div>
                    {sug.respuesta && (
                    ...
                    <div style={{ fontSize: 10, color: '#7E9389', marginTop: 3 }}>{sug.created_at.slice(0, 10)}</div>
                  </div>

...
export default function CierreAnualTab({ cierres, cuotas, gastos, proyectoId, companyId, moneda, autorNombre, canCreate, onRefresh }: Props) {
  ...
  async function firmar(c: CierreAnual) {
    const { isConfirmed } = await Swal.fire({
      ...
      html: '<p style="font-size:13px;color:#374151">Esta acción marca el cierre como definitivo. No podrá volver a estado borrador.</p>',
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ background: saldoAcumulado >= 0 ? '#eff6ff' : '#fef2f2', borderRadius: 10, padding: '12px 16px', textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: saldoAcumulado >= 0 ? '#2563eb' : '#ef4444' }}>
            {moneda} {Math.abs(saldoAcumulado).toLocaleString(undefined, { minimumFractionDigits: 2 })}
          </div>
          <div style={{ fontSize: 11, color: '#6b7280' }}>Saldo acumulado {saldoAcumulado < 0 ? '(déficit)' : '(superávit)'}</div>
        </div>
      </div>

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <span style={{ fontSize: 13, color: '#6b7280' }}>{cierres.length} cierre(s) registrado(s)</span>
        {canCreate && (
            ...
            {[...cierres].sort((a, b) => b.anio - a.anio).map(c => {
              ...
              return (
                <div key={c.id} onClick={() => setSelected(c === selected ? null : c)}
                  style={{ background: selected?.id === c.id ? '#f0fdfa' : '#fff', border: `1.5px solid ${selected?.id === c.id ? '#0d9488' : '#e5e7eb'}`, borderRadius: 10, padding: '14px 16px', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                      <div style={{ fontSize: 22, fontWeight: 800, color: '#0f172a' }}>{c.anio}</div>
                    ...
                    <div style={{ display: 'flex', gap: 20, fontSize: 12, color: '#6b7280' }}>
                      ...
                      <span>Saldo: <strong style={{ color: c.saldo >= 0 ? '#2563eb' : '#ef4444' }}>{moneda} {c.saldo.toFixed(2)}</strong></span>

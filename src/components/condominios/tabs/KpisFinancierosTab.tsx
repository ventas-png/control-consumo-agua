  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        ...
        {kpiCard('Saldo operativo', `${moneda} ${Math.abs(saldoOperativo).toFixed(2)}`, saldoOperativo >= 0 ? 'Superávit' : 'Déficit', saldoOperativo >= 0 ? 'var(--at-primary-tint)' : '#fef2f2', saldoOperativo >= 0 ? 'var(--at-primary)' : '#ef4444')}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        ...
        <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 16 }}>
          ...
          {[
            ...
            { label: '31–60 días', items: aging60, color: '#f97316' },
            { label: '61–90 días', items: aging90, color: '#ef4444' },
            { label: '+90 días', items: aging90plus, color: '#7f1d1d' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginTop: 16 }}>
        {[
          { label: 'Pagadas', val: cuotasFiltradas.filter(c => c.estado === 'pagado').length, monto: totalCobrado, bg: '#f0fdf4', color: '#16a34a' },
          { label: 'Pendientes', val: cuotasFiltradas.filter(c => c.estado === 'pendiente').length, monto: totalPendiente, bg: '#fffbeb', color: '#d97706' },
          { label: 'Morosas', val: cuotasFiltradas.filter(c => c.estado === 'moroso').length, monto: totalMoroso, bg: '#fef2f2', color: '#ef4444' },
        ].map(k => (

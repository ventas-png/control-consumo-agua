    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 13, color: '#6b7280' }}>{lista.length} visitas frecuentes</span>
        </div>
        {canCreate && (
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {DIAS.map(d => (
                <button key={d} type="button" onClick={() => toggleDia(d)}
                  style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid', fontSize: 12, cursor: 'pointer', background: form.dias_permitidos.includes(d) ? '#6366f1' : '#fff', color: form.dias_permitidos.includes(d) ? '#fff' : '#374151', borderColor: form.dias_permitidos.includes(d) ? '#6366f1' : '#d1d5db' }}>
                  {DIAS_LABEL[d]}
                </button>
              ))}
            </div>
      ...
      {lista.length === 0 ? (
        <div style={{ textAlign: 'center', color: '#9ca3af', padding: '40px 0', fontSize: 13 }}>Sin visitas frecuentes registradas</div>
      ) : (
        Object.entries(porUnidad).map(([unidadId, items]) => {
          const unidad = unidades.find(u => u.id === unidadId)
          return (
            <div key={unidadId} style={{ marginBottom: 20 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: '#374151', marginBottom: 8, paddingBottom: 4, borderBottom: '2px solid #e5e7eb' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10 }}>
                {items.map(v => {
                  ...
                  return (
                    <div key={v.id} style={{ background: v.activo ? '#fff' : '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 10, padding: '12px 14px', opacity: v.activo ? 1 : 0.6 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                          ...
                          {v.telefono && <div style={{ fontSize: 12, color: '#6b7280' }}>📞 {v.telefono}</div>}
                          {v.placa_vehiculo && <div style={{ fontSize: 12, color: '#6b7280' }}>🚗 {v.placa_vehiculo}</div>}
                          {v.dias_permitidos && v.dias_permitidos.length > 0 && (
                            <div style={{ display: 'flex', gap: 3, marginTop: 4, flexWrap: 'wrap' }}>
                              {DIAS.map(d => (
                                <span key={d} style={{ padding: '2px 5px', borderRadius: 4, fontSize: 10, background: v.dias_permitidos!.includes(d) ? '#eef2ff' : '#f3f4f6', color: v.dias_permitidos!.includes(d) ? '#6366f1' : '#d1d5db', fontWeight: v.dias_permitidos!.includes(d) ? 700 : 400 }}>
                                  {DIAS_LABEL[d]}
                                </span>
                              ))}
                            </div>
                          ...
                          {(v.hora_desde || v.hora_hasta) && (
                            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 3 }}>⏰ {v.hora_desde || '00:00'} – {v.hora_hasta || '23:59'}</div>
                          )}
                        </div>
                        {canEdit && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            <button onClick={() => toggleActivo(v)} title={v.activo ? 'Desactivar' : 'Activar'}
                              style={{ padding: '4px 8px', background: v.activo ? '#f3f4f6' : '#d1fae5', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 11 }}>
                              {v.activo ? '⏸' : '▶'}
                            </button>
                      ...
                      {v.notas && <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 6, fontStyle: 'italic' }}>{v.notas}</div>}
                    </div>
                  )
                })}
              </div>

...
const TIPO_STYLE: Record<string, { bg: string; color: string; label: string; icon: string }> = {
  remodelacion: { bg: '#f3e8ff', color: '#7c3aed', label: 'Remodelación', icon: '🔨' },
  ampliacion:   { bg: '#fef3c7', color: '#92400e', label: 'Ampliación',   icon: '📐' },
  reparacion:   { bg: '#e0f2fe', color: '#0369a1', label: 'Reparación',   icon: '🔧' },
...
const ESTADO_FLOW: Record<string, { bg: string; color: string; label: string; next?: string; nextLabel?: string }> = {
  solicitado:   { bg: '#e0f2fe', color: '#0369a1', label: 'Solicitado',   next: 'aprobado',      nextLabel: 'Aprobar' },
    ...
    <div style={{ display: 'flex', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ width: '360px', flexShrink: 0, borderRight: '1.5px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px', color: '#94a3b8', fontSize: '13px' }}>Sin permisos</div>
          )}
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} onClick={() => { setSelected(p); setShowForm(false) }}
                style={{ padding: '12px 14px', borderBottom: '1px solid #f1f5f9', cursor: 'pointer', background: selected?.id === p.id ? '#f0f9ff' : 'white', borderLeft: selected?.id === p.id ? '3px solid #0ea5e9' : '3px solid transparent' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '13px', color: '#0f172a', display: 'flex', gap: '6px', alignItems: 'center' }}>
                      <span>{ts.icon}</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        {showForm && (
          <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '12px' }}>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Tipo de obra *</label>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Horario permitido</label>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Fianza ({moneda})</label>
            ...
            <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <h2 style={{ margin: 0, fontSize: '17px', fontWeight: 700, color: '#0f172a' }}>{selected.descripcion}</h2>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#64748b' }}>Unidad: {unidad?.nombre ?? '—'}</p>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
              ...
              {selected.observaciones && (
                <div style={{ background: '#f8fafc', borderRadius: '8px', padding: '12px', marginBottom: '16px' }}>
                  <div style={{ fontSize: '11px', fontWeight: 600, color: '#94a3b8', marginBottom: '4px' }}>Observaciones</div>
                  <p style={{ margin: 0, fontSize: '13px', color: '#374151' }}>{selected.observaciones}</p>
                </div>
              )}
              ...
              {canEdit && (
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {es.next && (
                    <button onClick={() => avanzarEstado(selected)} style={{ padding: '8px 18px', background: '#0ea5e9', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '13px' }}>
                      → {es.nextLabel}
                    </button>
                  )}
        ...
        {!showForm && !selected && (
          <div style={{ textAlign: 'center', padding: '60px', color: '#94a3b8' }}>
            ...
            <p style={{ fontWeight: 600, color: '#64748b' }}>Selecciona un permiso o crea uno nuevo</p>
          </div>
        )}
      </div>
    </div>

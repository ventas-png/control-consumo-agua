...
const TIPO_STYLE: Record<string, { bg: string; color: string; label: string; icon: string }> = {
  remodelacion: { bg: '#F4EBE3', color: '#9C5733', label: 'Remodelación', icon: '🔨' },
  ampliacion:   { bg: '#fef3c7', color: '#92400e', label: 'Ampliación',   icon: '📐' },
  reparacion:   { bg: '#D9E2DC', color: '#102622', label: 'Reparación',   icon: '🔧' },
...
const ESTADO_FLOW: Record<string, { bg: string; color: string; label: string; next?: string; nextLabel?: string }> = {
  solicitado:   { bg: '#D9E2DC', color: '#102622', label: 'Solicitado',   next: 'aprobado',      nextLabel: 'Aprobar' },
    ...
    <div style={{ display: 'flex', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ width: '360px', flexShrink: 0, borderRight: '1.5px solid #E1DDD0', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px', color: '#7E9389', fontSize: '13px' }}>Sin permisos</div>
          )}
          {filtered.map(p => {
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        {showForm && (
          <div style={{ background: '#FAF7EF', border: '1.5px solid #E1DDD0', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '12px' }}>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '3px' }}>Tipo de obra *</label>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '3px' }}>Horario permitido</label>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', display: 'block', marginBottom: '3px' }}>Fianza ({moneda})</label>
          ...
          return (
            <div style={{ background: 'white', border: '1.5px solid #E1DDD0', borderRadius: '12px', padding: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <h2 style={{ margin: 0, fontSize: '17px', fontWeight: 700, color: '#15291F' }}>{selected.descripcion}</h2>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#7E9389' }}>Unidad: {unidad?.nombre ?? '—'}</p>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
              ...
              {selected.observaciones && (
                <div style={{ background: '#FAF7EF', borderRadius: '8px', padding: '12px', marginBottom: '16px' }}>
                  <div style={{ fontSize: '11px', fontWeight: 600, color: '#7E9389', marginBottom: '4px' }}>Observaciones</div>
                  <p style={{ margin: 0, fontSize: '13px', color: '#3E5A4C' }}>{selected.observaciones}</p>
                </div>
              )}
              ...
              {canEdit && (
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {es.next && (
                    <button onClick={() => avanzarEstado(selected)} style={{ padding: '8px 18px', background: '#1B3B36', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '13px' }}>
                      → {es.nextLabel}
                    </button>
                  )}
        ...
        {!showForm && !selected && (
          <div style={{ textAlign: 'center', padding: '60px', color: '#7E9389' }}>
            ...
            <p style={{ fontWeight: 600, color: '#7E9389' }}>Selecciona un permiso o crea uno nuevo</p>
          </div>
        )}
      </div>
    </div>

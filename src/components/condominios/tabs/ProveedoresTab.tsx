  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Contratos de Proveedores</h2>
          {totalMensual > 0 && (
            <span style={{ fontSize: '12px', color: '#64748b' }}>
              Total mensual activos: <strong style={{ color: '#0ea5e9' }}>{moneda} {totalMensual.toFixed(2)}</strong>
            </span>
          )}
        </div>
        {canCreate && !showForm && (

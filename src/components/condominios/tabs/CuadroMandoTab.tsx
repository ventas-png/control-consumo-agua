    ...
    <div style={{ background: '#fff', border: `1px solid ${k.color}33`, borderRadius: 10, padding: '12px 14px', borderTop: `3px solid ${k.color}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          {k.sub && <div style={{ fontSize: 9, color: '#7E9389', marginTop: 2 }}>{k.sub}</div>}
        </div>
...
export default function CuadroMandoTab({ cuotas, tickets, visitantes, gastos, presupuestos, incidentes, sugerencias, polizas, contratosProveedores, inspecciones, vencimientosExtra, encuestas, moneda, proyectoNombre = 'Condominio' }: Props) {
  ...
  const kpisOperacion: KpiCard[] = useMemo(() => {
    ...
    return [
      ...
      { label: 'Visitantes del mes', val: visitantesMes, color: '#1B3B36', sub: mes, icon: '🚪' },
  ...
  const vencimientos30 = useMemo(() => [
    ...polizas.filter(p => p.fecha_vencimiento && p.fecha_vencimiento >= hoy && p.fecha_vencimiento <= en30).map(p => ({ titulo: `Póliza: ${p.tipo}`, fecha: p.fecha_vencimiento, tipo: 'Seguro', color: '#9C5733' })),
    ...contratosProveedores.filter(c => c.fecha_fin && c.fecha_fin >= hoy && c.fecha_fin <= en30).map(c => ({ titulo: `Contrato: ${c.proveedor_nombre}`, fecha: c.fecha_fin!, tipo: 'Contrato', color: '#102622' })),
    ...
    ...vencimientosExtra.filter(v => !v.renovado && v.fecha_vencimiento >= hoy && v.fecha_vencimiento <= en30).map(v => ({ titulo: v.titulo, fecha: v.fecha_vencimiento, tipo: 'Extra', color: '#7E9389' })),
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {/* Sección financiera */}
      <div style={{ fontWeight: 700, fontSize: 12, color: '#7E9389', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Finanzas y cobro</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        {kpisFinanciero.map(k => <KpiBox key={k.label} k={k} />)}
      </div>

      {/* Sección operativa */}
      <div style={{ fontWeight: 700, fontSize: 12, color: '#7E9389', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Operaciones</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #E1DDD0', borderRadius: 12, padding: 16 }}>
          ...
          {[
            ...
            <div key={r.label} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              ...
              <div style={{ flex: 1, height: 12, background: '#EAE6D8', borderRadius: 6, overflow: 'hidden' }}>
          ...
          {Object.values(ticketsPorPrioridad).every(v => v === 0) && (
            <div style={{ fontSize: 12, color: '#7E9389', textAlign: 'center', padding: '16px 0' }}>Sin tickets abiertos ✓</div>
          )}
        </div>
          ...
          {vencimientos30.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 220, overflowY: 'auto' }}>
              {vencimientos30.map((v, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 8px', background: dias <= 7 ? '#fef2f2' : '#FAF7EF', borderRadius: 7 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: '#15291F', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.titulo}</div>
                      <div style={{ fontSize: 10, color: '#7E9389' }}>{v.fecha} · {v.tipo}</div>
                    </div>
                  </div>
                )
              })}
        ...
        <div style={{ marginTop: 16, background: '#fffbeb', borderRadius: 10, padding: '12px 16px', border: '1px solid #fde68a' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {recomendaciones.map((r, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '5px 8px', background: r.prioridad === 'alta' ? '#fef2f2' : '#fff', borderRadius: 7, border: `1px solid ${r.prioridad === 'alta' ? '#fecaca' : '#E1DDD0'}` }}>
                <span style={{ fontSize: 14 }}>{r.icono}</span>
                <span style={{ fontSize: 11, color: r.prioridad === 'alta' ? '#b91c1c' : '#3E5A4C', fontWeight: r.prioridad === 'alta' ? 700 : 500 }}>{r.texto}</span>
          ...
          {[
            ...
            return (
              <div key={item.area} style={{ textAlign: 'center', padding: '8px 4px' }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 600, color: '#3E5A4C', marginTop: 2 }}>{item.area}</div>
                <div style={{ fontSize: 10, color: '#7E9389' }}>{item.score}{item.invert ? '%' : ''}</div>
              </div>
            )
          })}
        </div>

...
export default function SimuladorCuotasTab({ cuotas, gastos, unidades, moneda }: Props) {
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', marginBottom: 2 }}>Simulador de Cuotas</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 16 }}>
        ...
        <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: '#0f172a', marginBottom: 14 }}>Parámetros</div>

          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: '#374151', fontWeight: 600, display: 'block', marginBottom: 6 }}>
              Cuota mensual por unidad — <span style={{ color: '#2563eb' }}>{moneda} {cuotaMensual.toLocaleString('es')}</span>
            </label>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {[6, 12, 24].map(n => (
              <button key={n} onClick={() => setMesesProyeccion(n)}
                style={{ marginRight: 6, padding: '4px 12px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 12, cursor: 'pointer',
                  background: mesesProyeccion === n ? '#2563eb' : '#fff',
                  color: mesesProyeccion === n ? '#fff' : '#374151', fontWeight: mesesProyeccion === n ? 700 : 400 }}>
                {n}m
              </button>
            ))}
          </div>
          ...
          {/* Baseline info */}
          <div style={{ marginTop: 16, padding: '10px 12px', background: '#eff6ff', borderRadius: 8, fontSize: 11, color: '#1d4ed8' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {[
              ...
              { label: 'Punto de equilibrio', val: `${moneda} ${puntoEquilibrio.toLocaleString('es')}/unidad`, color: '#2563eb', bg: '#eff6ff' },
              { label: 'Potencial máximo/mes', val: `${moneda} ${Math.round(cuotaMensual * totalUnidades).toLocaleString('es')}`, color: '#7c3aed', bg: '#f5f3ff' },
            ].map(k => (
          ...
          {/* Gráfica */}
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 14, flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 12, color: '#0f172a', marginBottom: 12 }}>
              ...
              {proyeccion.map((p, i) => (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  <div style={{ width: '100%', display: 'flex', gap: 1, alignItems: 'flex-end', height: 120 }}>
                    <div style={{ flex: 1, background: '#2563eb', opacity: 0.8, borderRadius: '3px 3px 0 0',
                  ...
                  <div style={{ fontSize: 9, color: '#9ca3af', transform: 'rotate(-30deg)', marginTop: 4, whiteSpace: 'nowrap' }}>
                    {labelMes(p.mes)}
                  </div>
                </div>
              ))}
              ...
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Mes', 'Ingresos', 'Egresos', 'Superávit/Déficit', 'Acumulado'].map(h => (
                    <th key={h} style={{ padding: '7px 10px', textAlign: h === 'Mes' ? 'left' : 'right', color: '#64748b', fontWeight: 600 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {proyeccion.reduce<{ rows: React.ReactElement[]; acum: number }>((acc, p, i) => {
                  ...
                  acc.rows.push(
                    <tr key={i} style={{ borderTop: '1px solid #f1f5f9', background: p.superavit < 0 ? '#fef2f2' : undefined }}>
                      <td style={{ padding: '6px 10px', color: '#64748b' }}>{labelMes(p.mes)} {p.mes.slice(0, 4)}</td>

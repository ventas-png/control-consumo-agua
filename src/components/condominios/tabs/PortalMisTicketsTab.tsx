...
const ESTADO_CONFIG: Record<EstadoTicket, { label: string; icon: string; bg: string; color: string }> = {
  ...
  en_proceso: { label: 'En proceso',  icon: '🔵', bg: '#eff6ff', color: '#2563eb' },
  resuelto:   { label: 'Resuelto',    icon: '🟢', bg: '#f0fdf4', color: '#16a34a' },
  cerrado:    { label: 'Cerrado',     icon: '⚪', bg: '#f8fafc', color: '#64748b' },
}
...
const PRIORIDAD_CONFIG: Record<PrioridadTicket, { label: string; bg: string; color: string }> = {
  ...
  urgente: { label: 'Urgente', bg: '#fdf2f8', color: '#c026d3' },
}
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <h3 style={{ margin: 0, fontSize: '17px', fontWeight: 700, color: '#0f172a' }}>Mis solicitudes de mantenimiento</h3>
          {abiertos > 0 && <p style={{ margin: '3px 0 0', fontSize: '13px', color: '#2563eb' }}>{abiertos} solicitud{abiertos !== 1 ? 'es' : ''} en proceso</p>}
        </div>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', marginBottom: '18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>¿Qué necesita atención? *</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Descripción detallada</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '6px' }}>Urgencia</label>
                ...
                {(['baja', 'media', 'alta', 'urgente'] as PrioridadTicket[]).map(p => {
                  ...
                  return (
                    <button key={p} onClick={() => setForm(f => ({ ...f, prioridad: p }))}
                      style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '12.5px', fontWeight: 700, cursor: 'pointer', border: '1.5px solid', borderColor: form.prioridad === p ? pc.color : '#e2e8f0', background: form.prioridad === p ? pc.bg : 'white', color: form.prioridad === p ? pc.color : '#94a3b8' }}>
                      {pc.label}
                    </button>
                  )
                })}
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '14px' }}>
            <button onClick={enviarSolicitud} disabled={saving} style={{ padding: '10px 22px', background: 'linear-gradient(135deg,#2563eb,#1d4ed8)', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>
        ...
        {(['todos', 'abierto', 'en_proceso', 'resuelto', 'cerrado'] as const).map(f => (
          <button key={f} onClick={() => setFiltro(f)}
            style={{ padding: '5px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', border: 'none',
              background: filtro === f ? '#eff6ff' : 'transparent', color: filtro === f ? '#2563eb' : '#94a3b8' }}>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>
          ...
          <p style={{ fontWeight: 600, color: '#64748b' }}>Sin solicitudes{filtro !== 'todos' ? ' con este estado' : ''}</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.sort((a, b) => b.created_at.localeCompare(a.created_at)).map(t => {
            ...
            return (
              <div key={t.id} style={{ background: 'white', border: `1.5px solid ${t.estado === 'resuelto' || t.estado === 'cerrado' ? '#e2e8f0' : '#bfdbfe'}`, borderRadius: '12px', padding: '14px 16px' }}>
                <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                  <span style={{ fontSize: '20px', flexShrink: 0 }}>{ec.icon}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '14px', color: '#0f172a', marginBottom: '3px' }}>{t.titulo}</div>
                    {t.descripcion && <div style={{ fontSize: '12.5px', color: '#64748b', marginBottom: '5px' }}>{t.descripcion}</div>}
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '11.5px', color: '#94a3b8' }}>{new Date(t.created_at).toLocaleDateString('es', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                    </div>

  ...
  return (
    <div style={{ display: 'flex', height: '100%', gap: 0 }}>
      ...
      <div style={{ width: 300, borderRight: '1px solid #e5e7eb', overflowY: 'auto', flexShrink: 0 }}>
        ...
        {proximos.length > 0 && (
          <div style={{ padding: '8px 12px', background: '#f0fdf4', borderBottom: '1px solid #e5e7eb' }}>
            ...
            {proximos.slice(0, 3).map(r => {
              ...
              return (
                <div key={r.id} style={{ fontSize: 11, color: '#374151', marginBottom: 2 }}>
                  ...
                  {r.proveedor && <span style={{ color: '#6b7280' }}> · {r.proveedor}</span>}
                </div>
              )
            })}
          </div>
        ...
        {lista.map(r => {
          ...
          return (
            <div key={r.id} onClick={() => { setSelected(r); setMostrarForm(false) }}
              style={{ padding: '10px 12px', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', background: selected?.id === r.id ? '#f0fdf4' : '#fff', borderLeft: `3px solid ${tipo?.color}` }}>
              ...
              <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>{r.fecha}{r.proveedor ? ` · ${r.proveedor}` : ''}</div>
              <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>{r.areas.slice(0, 2).join(', ')}{r.areas.length > 2 ? ` +${r.areas.length - 2}` : ''}</div>
            </div>
          )
        })}
      </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ background: '#eef2ff', borderRadius: 10, padding: '12px 16px' }}>
                <div style={{ fontSize: 11, color: '#6366f1' }}>Total registros</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ gridColumn: 'span 3' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 6 }}>
                  {AREAS_COMUNES.map(a => (
                    <button key={a} type="button" onClick={() => toggleArea(a)}
                      style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid', fontSize: 11, cursor: 'pointer', background: form.areas.includes(a) ? '#22c55e' : '#fff', color: form.areas.includes(a) ? '#fff' : '#374151', borderColor: form.areas.includes(a) ? '#22c55e' : '#d1d5db' }}>
                      {a}
                    </button>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button type="button" onClick={agregarAreaCustom} style={{ padding: '7px 12px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: 6, cursor: 'pointer', fontSize: 12 }}>+</button>
                </div>
              </div>
              <div style={{ gridColumn: 'span 3' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div>
                  ...
                  {selected.proveedor && <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>🤝 {selected.proveedor}</div>}
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
                {selected.trabajadores != null && (
                  <div style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 14px' }}>
                    <div style={{ fontSize: 11, color: '#9ca3af' }}>Trabajadores</div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>{selected.trabajadores}</div>
                  </div>
                )}
                {selected.horas_trabajo != null && (
                  <div style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 14px' }}>
                    <div style={{ fontSize: 11, color: '#9ca3af' }}>Horas</div>
              ...
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 6 }}>ÁREAS TRATADAS</div>
              ...
              {selected.insumos && (
                <div style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 14px', marginBottom: 12, fontSize: 13 }}>
                  <span style={{ fontSize: 11, color: '#9ca3af' }}>Insumos: </span>{selected.insumos}
                </div>
              )}

              {selected.proxima_visita && (
                <div style={{ background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: 8, padding: '10px 14px', marginBottom: 12, fontSize: 13, color: '#6366f1' }}>
                  📅 Próxima visita: <strong>{selected.proxima_visita}</strong>
                </div>
              )}

              {selected.observaciones && (
                <div style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#374151', marginBottom: 12 }}>
                  {selected.observaciones}
                </div>
              )}

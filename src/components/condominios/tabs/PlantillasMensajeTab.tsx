        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <select value={filtroCanal} onChange={e => setFiltroCanal(e.target.value as CanalPlantilla | '')}
              style={{ padding: '4px 8px', border: '1px solid var(--at-line)', borderRadius: 6, fontSize: 11 }}>
        ...
        {filtradas.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filtradas.map(p => {
              ...
              return (
                <div key={p.id} style={{ background: 'var(--at-surface)', border: `1px solid ${p.activa ? c.color + '33' : 'var(--at-line)'}`, borderRadius: 10, padding: '10px 14px', opacity: p.activa ? 1 : 0.6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        ...
                        {!p.activa && <span style={{ fontSize: 10, color: 'var(--at-ink-3)' }}>Inactiva</span>}
                      </div>
                      <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--at-ink)', marginBottom: 2 }}>{p.nombre}</div>
                      <div style={{ fontSize: 11, color: 'var(--at-ink-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.cuerpo.slice(0, 80)}{p.cuerpo.length > 80 ? '…' : ''}</div>
                      {p.variables.length > 0 && (
                        <div style={{ marginTop: 4, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                          {p.variables.map(v => (
                            <span key={v} style={{ fontSize: 9, background: 'var(--at-chip)', color: 'var(--at-ink-3)', padding: '1px 5px', borderRadius: 4 }}>{v}</span>
                          ))}
                        </div>
                      )}
                    </div>
        ...
        {preview ? (
          <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <button onClick={() => setPreview(null)} style={{ fontSize: 12, padding: '3px 10px', border: '1px solid var(--at-line)', borderRadius: 6, cursor: 'pointer', background: 'var(--at-surface-2)' }}>✕ Cerrar</button>
            </div>
            ...
            {preview.asunto && <div style={{ fontSize: 12, color: 'var(--at-ink-3)', marginBottom: 8 }}>Asunto: {preview.asunto}</div>}
            <div style={{ background: 'var(--at-surface-2)', borderRadius: 8, padding: '12px 14px', fontSize: 13, color: 'var(--at-ink)', whiteSpace: 'pre-wrap', lineHeight: 1.6, border: '1px solid var(--at-line)' }}>
              {preview.cuerpo}
            </div>
            {preview.variables.length > 0 && (
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-3)', marginBottom: 4 }}>Variables detectadas:</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {preview.variables.map(v => (
                    <span key={v} style={{ fontSize: 10, background: 'var(--at-chip)', color: 'var(--at-ink-2)', padding: '2px 8px', borderRadius: 6, border: '1px solid var(--at-line)' }}>{v}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div style={{ background: 'var(--at-surface)', border: '1px solid var(--at-line)', borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-2)', display: 'block', marginBottom: 4 }}>Nombre *</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-2)', display: 'block', marginBottom: 4 }}>Canal</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-2)', display: 'block', marginBottom: 4 }}>Estado</label>
            ...
            {form.canal === 'email' && (
              <div style={{ marginBottom: 10 }}>
                <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-2)', display: 'block', marginBottom: 4 }}>Asunto</label>
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-2)', display: 'block', marginBottom: 4 }}>Cuerpo del mensaje *</label>
              <textarea value={form.cuerpo} onChange={e => setForm(f => ({ ...f, cuerpo: e.target.value }))}
                ...
                style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--at-line-strong)', borderRadius: 7, fontSize: 13, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }} />
            </div>

            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--at-ink-2)', marginBottom: 6 }}>Variables sugeridas</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {VARIABLES_SUGERIDAS.map(v => (
                  <button key={v} onClick={() => insertarVariable(v)}
                    style={{ fontSize: 10, padding: '3px 8px', background: 'var(--at-chip)', border: '1px solid var(--at-line)', borderRadius: 5, cursor: 'pointer', color: 'var(--at-ink-2)' }}>
                    {v}
                  </button>
                ))}
              </div>
            ...
            {extraerVariables(form.cuerpo).length > 0 && (
              <div style={{ marginBottom: 14, padding: '8px 12px', background: 'var(--at-primary-tint)', borderRadius: 8, fontSize: 11, color: 'var(--at-primary-hover)' }}>
          ...
          <div style={{ textAlign: 'center', color: 'var(--at-ink-3)', padding: '60px 0', fontSize: 13 }}>

                ...
                <div key={p.id} style={{ background: '#fff', border: `1px solid ${p.activa ? c.color + '33' : '#E1DDD0'}`, borderRadius: 10, padding: '10px 14px', opacity: p.activa ? 1 : 0.6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', gap: 4, flexShrink: 0, marginLeft: 8 }}>
                      <button onClick={() => setPreview(p)} style={{ fontSize: 11, padding: '3px 8px', border: '1px solid var(--at-line)', borderRadius: 5, cursor: 'pointer', background: '#FAF7EF' }}>👁</button>
                      {canEdit && <button onClick={() => abrirEditar(p)} style={{ fontSize: 11, padding: '3px 8px', border: '1px solid var(--at-line)', borderRadius: 5, cursor: 'pointer', background: '#FAF7EF' }}>✏️</button>}
                      {canEdit && <button onClick={() => toggleActiva(p)} style={{ fontSize: 11, padding: '3px 8px', border: '1px solid var(--at-line)', borderRadius: 5, cursor: 'pointer', background: '#FAF7EF' }}>{p.activa ? '⏸' : '▶'}</button>}
        ...
        {preview ? (
          <div style={{ background: '#fff', border: '1px solid var(--at-line)', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <button onClick={() => setPreview(null)} style={{ fontSize: 12, padding: '3px 10px', border: '1px solid var(--at-line)', borderRadius: 6, cursor: 'pointer', background: '#FAF7EF' }}>✕ Cerrar</button>
            </div>
            ...
            <div style={{ background: '#FAF7EF', borderRadius: 8, padding: '12px 14px', fontSize: 13, color: '#15291F', whiteSpace: 'pre-wrap', lineHeight: 1.6, border: '1px solid var(--at-line)' }}>
              {preview.cuerpo}
            </div>
              ...
              <div style={{ marginTop: 10 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {preview.variables.map(v => (
                    <span key={v} style={{ fontSize: 10, background: '#EAE6D8', color: '#3E5A4C', padding: '2px 8px', borderRadius: 6, border: '1px solid var(--at-line)' }}>{v}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {VARIABLES_SUGERIDAS.map(v => (
                  <button key={v} onClick={() => insertarVariable(v)}
                    style={{ fontSize: 10, padding: '3px 8px', background: '#EAE6D8', border: '1px solid var(--at-line)', borderRadius: 5, cursor: 'pointer', color: '#3E5A4C' }}>
                    {v}
                  </button>
                ))}
              </div>

...
const CANAL_CFG: Record<CanalPlantilla, { label: string; color: string; bg: string; icon: string }> = {
  ...
  sms:      { label: 'SMS',      color: '#7c3aed', bg: '#f5f3ff', icon: '📱' },
}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {canCreate && (
              <button onClick={abrirNueva}
                style={{ padding: '5px 12px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                + Nueva
              </button>
            )}
          </div>
        ...
        {filtradas.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filtradas.map(p => {
              ...
              return (
                <div key={p.id} style={{ background: '#fff', border: `1px solid ${p.activa ? c.color + '33' : '#e5e7eb'}`, borderRadius: 10, padding: '10px 14px', opacity: p.activa ? 1 : 0.6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        ...
                        {!p.activa && <span style={{ fontSize: 10, color: '#9ca3af' }}>Inactiva</span>}
                      </div>
                      <div style={{ fontWeight: 600, fontSize: 13, color: '#0f172a', marginBottom: 2 }}>{p.nombre}</div>
                      <div style={{ fontSize: 11, color: '#6b7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.cuerpo.slice(0, 80)}{p.cuerpo.length > 80 ? '…' : ''}</div>
                      {p.variables.length > 0 && (
                        <div style={{ marginTop: 4, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                          {p.variables.map(v => (
                            <span key={v} style={{ fontSize: 9, background: '#f1f5f9', color: '#64748b', padding: '1px 5px', borderRadius: 4 }}>{v}</span>
                          ))}
                        </div>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: 4, flexShrink: 0, marginLeft: 8 }}>
                      <button onClick={() => setPreview(p)} style={{ fontSize: 11, padding: '3px 8px', border: '1px solid #e5e7eb', borderRadius: 5, cursor: 'pointer', background: '#f9fafb' }}>👁</button>
                      {canEdit && <button onClick={() => abrirEditar(p)} style={{ fontSize: 11, padding: '3px 8px', border: '1px solid #e5e7eb', borderRadius: 5, cursor: 'pointer', background: '#f9fafb' }}>✏️</button>}
                      {canEdit && <button onClick={() => toggleActiva(p)} style={{ fontSize: 11, padding: '3px 8px', border: '1px solid #e5e7eb', borderRadius: 5, cursor: 'pointer', background: '#f9fafb' }}>{p.activa ? '⏸' : '▶'}</button>}
        ...
        {preview ? (
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <button onClick={() => setPreview(null)} style={{ fontSize: 12, padding: '3px 10px', border: '1px solid #e5e7eb', borderRadius: 6, cursor: 'pointer', background: '#f9fafb' }}>✕ Cerrar</button>
            </div>
            ...
            {preview.asunto && <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 8 }}>Asunto: {preview.asunto}</div>}
            <div style={{ background: '#f9fafb', borderRadius: 8, padding: '12px 14px', fontSize: 13, color: '#0f172a', whiteSpace: 'pre-wrap', lineHeight: 1.6, border: '1px solid #e5e7eb' }}>
              {preview.cuerpo}
            </div>
            {preview.variables.length > 0 && (
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', marginBottom: 4 }}>Variables detectadas:</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {preview.variables.map(v => (
                    <span key={v} style={{ fontSize: 10, background: '#f1f5f9', color: '#475569', padding: '2px 8px', borderRadius: 6, border: '1px solid #e2e8f0' }}>{v}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Canal</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Estado</label>
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Cuerpo del mensaje *</label>
              <textarea value={form.cuerpo} onChange={e => setForm(f => ({ ...f, cuerpo: e.target.value }))}
                ...
                style={{ width: '100%', padding: '7px 10px', border: '1px solid #d1d5db', borderRadius: 7, fontSize: 13, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }} />
            </div>

            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Variables sugeridas</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {VARIABLES_SUGERIDAS.map(v => (
                  <button key={v} onClick={() => insertarVariable(v)}
                    style={{ fontSize: 10, padding: '3px 8px', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 5, cursor: 'pointer', color: '#475569' }}>
                    {v}
                  </button>
                ))}
              </div>
            ...
            {extraerVariables(form.cuerpo).length > 0 && (
              <div style={{ marginBottom: 14, padding: '8px 12px', background: '#f0f9ff', borderRadius: 8, fontSize: 11, color: '#0369a1' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={guardar} disabled={saving}
                style={{ padding: '8px 18px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 7, cursor: 'pointer', fontSize: 13, fontWeight: 600, opacity: saving ? 0.7 : 1 }}>

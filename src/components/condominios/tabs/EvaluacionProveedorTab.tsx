    ...
    <div style={{ display: 'flex', gap: '2px' }}>
      {[1, 2, 3, 4, 5].map(n => (
        <span key={n} onClick={() => !readOnly && onChange?.(n)}
          style={{ fontSize: readOnly ? '14px' : '18px', cursor: readOnly ? 'default' : 'pointer', color: n <= value ? '#f59e0b' : '#e2e8f0', lineHeight: 1 }}>
          ★
        </span>
      ))}
    </div>
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: '0 0 2px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Evaluación de Proveedores</h2>
          <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8' }}>{evaluaciones.length} evaluaciones · {ranking.length} proveedores</p>
        </div>
            ...
            {(['lista', 'ranking'] as const).map(v => (
              <button key={v} onClick={() => setView(v)}
                style={{ padding: '6px 12px', border: 'none', fontSize: '12px', cursor: 'pointer', fontWeight: view === v ? 700 : 500,
                  background: view === v ? '#0ea5e9' : 'white', color: view === v ? 'white' : '#64748b' }}>
      ...
      {showForm && (
        <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px', marginBottom: '12px' }}>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Proveedor (contrato)</label>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Nombre proveedor *</label>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Fecha evaluación</label>
              <input style={inputStyle} type="date" value={form.fecha} onChange={e => setF('fecha', e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Evaluado por</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '12px', marginBottom: '12px' }}>
            {([
              ...
              <div key={k}>
                <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '5px' }}>{label}</label>
                <Stars value={form[k] as number} onChange={n => setF(k, n)} />
                <div style={{ fontSize: '10px', color: '#94a3b8', marginTop: '2px' }}>{form[k]}/5</div>
              </div>
            ))}
          </div>
          <div style={{ marginBottom: '12px' }}>
            <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Comentarios</label>
      ...
      {view === 'ranking' ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {ranking.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontSize: '13px' }}>Sin evaluaciones.</div>
          ) : ranking.map((r, i) => (
            <div key={r.nombre} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '10px', padding: '12px 14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                <div>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '4px' }}>
                    <span style={{ fontSize: '16px', fontWeight: 800, color: i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : i === 2 ? '#b45309' : '#64748b' }}>#{i + 1}</span>
                    <span style={{ fontWeight: 700, fontSize: '14px' }}>{r.nombre}</span>
                    <span style={{ fontSize: '10px', color: '#94a3b8' }}>{r.count} evaluación(es)</span>
                  </div>
        ...
        <>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontSize: '13px' }}>No hay evaluaciones.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {filtered.map(e => (
                <div key={e.id} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '10px', padding: '12px 14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      ...
                      {e.comentarios && <div style={{ fontSize: '12px', color: '#374151', marginTop: '4px', fontStyle: 'italic' }}>{e.comentarios}</div>}
                    </div>
                    {canEdit && (

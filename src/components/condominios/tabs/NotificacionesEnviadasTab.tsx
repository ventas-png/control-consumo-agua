...
const ESTADO_CFG: Record<EstadoNotificacion, { label: string; bg: string; color: string }> = {
  ...
  leido:     { label: 'Leído',     bg: '#e0f2fe', color: '#0369a1' },
}
    ...
    <div style={{ padding: 16 }}>
          ...
          {resumen.map(({ canal, count, fallidos }) => {
            ...
            return (
              <button key={canal} onClick={() => setFiltroCanal(filtroCanal === canal ? '' : canal)}
                style={{ padding: '10px 16px', borderRadius: 10, border: `1.5px solid ${filtroCanal === canal ? cfg.color : '#e5e7eb'}`, background: filtroCanal === canal ? cfg.bg : '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 18 }}>{cfg.icon}</span>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: cfg.color }}>{count}</div>
                  <div style={{ fontSize: 10, color: '#6b7280' }}>{cfg.label}{fallidos > 0 ? ` · ${fallidos} fallidos` : ''}</div>
                </div>
              </button>
            )
          })}
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
        <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar por destinatario, asunto…"
          style={{ flex: 1, minWidth: 180, padding: '7px 10px', border: '1px solid #d1d5db', borderRadius: 7, fontSize: 13 }} />
      ...
      {lista.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {lista.map(n => {
            ...
            return (
              <div key={n.id} style={{ background: '#fff', border: `1px solid ${n.estado === 'fallido' ? '#fecaca' : '#e5e7eb'}`, borderRadius: 10, overflow: 'hidden' }}>
                <div onClick={() => setExpanded(isOpen ? null : n.id)}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 600, fontSize: 13, color: '#0f172a' }}>{n.destinatario}</span>
                      {n.asunto && <span style={{ fontSize: 12, color: '#6b7280' }}>· {n.asunto}</span>}
                ...
                {isOpen && (
                  <div style={{ padding: '10px 14px', borderTop: '1px solid #f3f4f6', background: '#f9fafb' }}>
                    <div style={{ fontSize: 13, color: '#374151', whiteSpace: 'pre-wrap', lineHeight: 1.5, maxHeight: 200, overflowY: 'auto' }}>{n.contenido}</div>
                    {n.error_detalle && (

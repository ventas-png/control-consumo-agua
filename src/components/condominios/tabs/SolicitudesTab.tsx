...
const ESTADO_STYLE: Record<EstadoSolicitud, { color: string; bg: string; label: string }> = {
  ...
  en_proceso: { color: '#0ea5e9', bg: '#e0f2fe', label: 'En Proceso' },
...
const PRIORIDAD_COLOR: Record<PrioridadSolicitud, string> = {
  baja: '#94a3b8', normal: '#0ea5e9', alta: '#f59e0b', urgente: '#ef4444',
}
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Solicitudes de Residentes</h2>
        {canCreate && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginBottom: '16px' }}>
        {[
          { label: 'Total',         value: solicitudes.length, color: '#0f172a' },
          { label: 'Pendientes',    value: pendientes,         color: '#f59e0b' },
          { label: 'En proceso',    value: enProceso,          color: '#0ea5e9' },
      ...
      {showForm && (
        <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: '10px' }}>
            ...
            <div>
              <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '3px' }}>Fecha límite</label>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
        <select value={filtroEstado} onChange={e => setFiltroEstado(e.target.value as EstadoSolicitud | 'all')}
          style={{ padding: '5px 10px', border: '1.5px solid #e2e8f0', borderRadius: '7px', fontSize: '12px', background: 'white', cursor: 'pointer' }}>
        ...
        <span style={{ fontSize: '12px', color: '#94a3b8', alignSelf: 'center' }}>{filtered.length} solicitud(es)</span>
      </div>

      {/* List */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontSize: '13px' }}>No hay solicitudes que mostrar.</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.map(s => {
            ...
            return (
              <div key={s.id} style={{ background: 'white', border: `1.5px solid ${isExpanded ? '#bfdbfe' : '#e2e8f0'}`, borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 14px', display: 'flex', gap: '12px', alignItems: 'flex-start', cursor: 'pointer' }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 700, fontSize: '13px', color: '#0f172a' }}>{tipo.label}</span>
                      {s.unidad_nombre && <span style={{ fontSize: '11px', color: '#64748b' }}>• {s.unidad_nombre}</span>}
                    ...
                    <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{s.created_at.slice(0,10)}{s.fecha_limite ? ` — límite: ${s.fecha_limite}` : ''}</div>
                  </div>
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid #f1f5f9', padding: '12px 14px', background: '#f8fafc' }}>
                    <p style={{ margin: '0 0 8px', fontSize: '13px', color: '#1e293b' }}>{s.descripcion}</p>
                    {s.respuesta && (
                      <div style={{ background: '#dcfce7', border: '1px solid #bbf7d0', borderRadius: '7px', padding: '8px 10px', marginBottom: '8px' }}>
                        ...
                        <div style={{ fontSize: '12px', color: '#1e293b' }}>{s.respuesta}</div>
                      </div>
                    )}
                    {canEdit && (s.estado === 'pendiente' || s.estado === 'en_proceso') && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <textarea value={respuesta} onChange={e => setRespuesta(e.target.value)}
                          ...
                          rows={2} style={{ padding: '7px 10px', border: '1.5px solid #e2e8f0', borderRadius: '7px', fontSize: '12px', resize: 'vertical', fontFamily: 'inherit' }} />
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                          {s.estado === 'pendiente' && (
                            <button onClick={() => handleEstado(s.id, 'en_proceso')}
                              style={{ padding: '5px 12px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
                              ▶ En Proceso
                            </button>
                          )}

...
const ESTADO_STYLE: Record<EstadoSolicitud, { color: string; bg: string; label: string }> = {
  ...
  en_proceso: { color: '#1B3B36', bg: '#D9E2DC', label: 'En Proceso' },
...
const PRIORIDAD_COLOR: Record<PrioridadSolicitud, string> = {
  baja: '#7E9389', normal: '#1B3B36', alta: '#f59e0b', urgente: '#ef4444',
}
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#15291F' }}>Solicitudes de Residentes</h2>
        {canCreate && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginBottom: '16px' }}>
        {[
          { label: 'Total',         value: solicitudes.length, color: '#15291F' },
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '12px', color: '#7E9389', alignSelf: 'center' }}>{filtered.length} solicitud(es)</span>
      </div>

      {/* List */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#7E9389', fontSize: '13px' }}>No hay solicitudes que mostrar.</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.map(s => {
            ...
            return (
              <div key={s.id} style={{ background: 'white', border: `1.5px solid ${isExpanded ? '#C2D2CA' : '#E1DDD0'}`, borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 14px', display: 'flex', gap: '12px', alignItems: 'flex-start', cursor: 'pointer' }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 700, fontSize: '13px', color: '#15291F' }}>{tipo.label}</span>
                      {s.unidad_nombre && <span style={{ fontSize: '11px', color: '#7E9389' }}>• {s.unidad_nombre}</span>}
                    ...
                    <div style={{ fontSize: '12px', color: '#7E9389', marginTop: '3px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {s.descripcion}
                    </div>
                    <div style={{ fontSize: '11px', color: '#7E9389', marginTop: '2px' }}>{s.created_at.slice(0,10)}{s.fecha_limite ? ` — límite: ${s.fecha_limite}` : ''}</div>
                  </div>
                  ...
                  <div style={{ borderTop: '1px solid #EAE6D8', padding: '12px 14px', background: '#FAF7EF' }}>
                    ...
                    {s.respuesta && (
                      <div style={{ background: '#dcfce7', border: '1px solid #bbf7d0', borderRadius: '7px', padding: '8px 10px', marginBottom: '8px' }}>
                        ...
                        <div style={{ fontSize: '12px', color: '#15291F' }}>{s.respuesta}</div>
                      </div>
                    )}
                        ...
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                          {s.estado === 'pendiente' && (
                            <button onClick={() => handleEstado(s.id, 'en_proceso')}
                              style={{ padding: '5px 12px', background: '#D9E2DC', color: '#102622', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
                              ▶ En Proceso
                            </button>
                          )}

...
export default function MetricasServicioTab({ tickets, sugerencias, visitantes, cuotas }: Props) {
  ...
  const ESTADO_COLOR: Record<string, string> = { abierto: '#ef4444', en_proceso: '#f97316', resuelto: '#4ade80', cerrado: '#94a3b8' }
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#0f172a' }}>Métricas de servicio del período</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        {kpiCard('Tickets del período', ticketsMes.length, `${tasaResolucionTickets}% resueltos`, '#7c3aed', '#f5f3ff')}
          ...
          {porPrioridad.map(({ prioridad, count }) => (
            <div key={prioridad} style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                <span style={{ textTransform: 'capitalize', color: '#374151' }}>{prioridad}</span>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {porEstado.map(({ estado, count }) => (
              <div key={estado} style={{ background: '#f9fafb', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
                <div style={{ fontSize: 18, fontWeight: 700, color: ESTADO_COLOR[estado] }}>{count}</div>
                <div style={{ fontSize: 10, color: '#6b7280', textTransform: 'capitalize' }}>{estado.replace('_', ' ')}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              {tendTickets.map(t => (
                <div key={t.periodo} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  <div style={{ width: '100%', display: 'flex', gap: 1, alignItems: 'flex-end', height: 90 }}>
                    <div style={{ flex: 1, background: '#a78bfa', borderRadius: '2px 2px 0 0', height: `${(t.total / maxTend) * 100}%`, minHeight: 2 }} title={`Tickets: ${t.total}`} />
                  ...
                  <div style={{ fontSize: 9, color: '#9ca3af' }}>{t.periodo.slice(2)}</div>
                </div>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 12, fontSize: 11, color: '#6b7280' }}>
              <span><span style={{ display: 'inline-block', width: 10, height: 10, background: '#a78bfa', borderRadius: 2, marginRight: 4 }} />Tickets</span>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
          ...
          {porCategoria.length === 0
            ? <div style={{ color: '#9ca3af', fontSize: 12, textAlign: 'center', padding: '20px 0' }}>Sin sugerencias</div>
            : porCategoria.map(({ cat, count }) => (
              <div key={cat} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                  <span style={{ color: '#374151' }}>{CAT_ICON[cat]} {cat}</span>
                  <span style={{ fontWeight: 600, color: '#6366f1' }}>{count}</span>
                </div>
                <div style={{ background: '#f3f4f6', borderRadius: 4, height: 6 }}>
                  <div style={{ height: '100%', background: '#a5b4fc', width: `${(count / maxCat) * 100}%`, borderRadius: 4 }} />
                </div>
              </div>
            ))
          }
          <div style={{ marginTop: 12, padding: '8px 12px', background: '#f5f3ff', borderRadius: 8, fontSize: 11, color: '#6b7280' }}>
            Respondidas: <strong style={{ color: '#6366f1' }}>{pct(sugerencias.filter(s => s.estado === 'respondida').length, sugerencias.length)}%</strong> del total
          </div>
        </div>
      </div>
    </div>

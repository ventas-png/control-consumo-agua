...
const ESTADO_CONFIG: Record<EstadoInfraccion, { label: string; bg: string; color: string }> = {
  ...
  anulada:      { label: 'Anulada',      bg: '#f8fafc', color: '#94a3b8' },
}
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>Infracciones</h2>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <button onClick={() => setFiltroEstado('todos')}
          style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: '1.5px solid', borderColor: filtroEstado === 'todos' ? '#0ea5e9' : '#e2e8f0', background: filtroEstado === 'todos' ? '#eff6ff' : 'white', color: filtroEstado === 'todos' ? '#0ea5e9' : '#64748b' }}>
        ...
        {(Object.entries(ESTADO_CONFIG) as [EstadoInfraccion, typeof ESTADO_CONFIG[EstadoInfraccion]][]).map(([e, cfg]) => (
          <button key={e} onClick={() => setFiltroEstado(e)}
            style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: '1.5px solid', borderColor: filtroEstado === e ? cfg.color : '#e2e8f0', background: filtroEstado === e ? cfg.bg : 'white', color: filtroEstado === e ? cfg.color : '#64748b' }}>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Unidad infractora *</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Monto multa ({moneda})</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Fecha infracción</label>
              <input type="date" value={form.fecha_infraccion} onChange={e => setForm(f => ({ ...f, fecha_infraccion: e.target.value }))}
                style={{ width: '100%', boxSizing: 'border-box', padding: '9px 12px', border: '1.5px solid #e2e8f0', borderRadius: '8px', fontSize: '14px', background: '#f8fafc' }} />
            </div>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '4px' }}>Fecha límite descargo</label>
      ...
      {filtradas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', color: '#94a3b8' }}>
          ...
          <p style={{ fontWeight: 600, color: '#64748b' }}>No hay infracciones registradas</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {filtradas.map(i => {
            ...
            return (
              <div key={i.id} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '14px', padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '6px', alignItems: 'center' }}>
                      <span style={{ fontWeight: 700, fontSize: '13px', color: '#374151' }}>{tipo.label}</span>
                      {i.unidad_nombre && <span style={{ fontSize: '12.5px', color: '#2563eb', fontWeight: 600 }}>📍 {i.unidad_nombre}</span>}
                    ...
                    <p style={{ margin: '0 0 6px', fontSize: '13.5px', color: '#374151' }}>{i.descripcion}</p>

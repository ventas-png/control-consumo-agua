...
const ESTADO_CONFIG: Record<EstadoInfraccion, { label: string; bg: string; color: string }> = {
  emitida:      { label: 'Emitida',      bg: '#EEF2EC', color: '#1B3B36' },
  ...
  anulada:      { label: 'Anulada',      bg: '#FAF7EF', color: '#7E9389' },
}
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#15291F' }}>Infracciones</h2>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <button onClick={() => setFiltroEstado('todos')}
          style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: '1.5px solid', borderColor: filtroEstado === 'todos' ? '#1B3B36' : '#E1DDD0', background: filtroEstado === 'todos' ? '#EEF2EC' : 'white', color: filtroEstado === 'todos' ? '#1B3B36' : '#7E9389' }}>
        ...
        {(Object.entries(ESTADO_CONFIG) as [EstadoInfraccion, typeof ESTADO_CONFIG[EstadoInfraccion]][]).map(([e, cfg]) => (
          <button key={e} onClick={() => setFiltroEstado(e)}
            style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: '1.5px solid', borderColor: filtroEstado === e ? cfg.color : '#E1DDD0', background: filtroEstado === e ? cfg.bg : 'white', color: filtroEstado === e ? cfg.color : '#7E9389' }}>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1px solid var(--at-line)', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#3E5A4C', display: 'block', marginBottom: '4px' }}>Unidad infractora *</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#3E5A4C', display: 'block', marginBottom: '4px' }}>Monto multa ({moneda})</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#3E5A4C', display: 'block', marginBottom: '4px' }}>Fecha infracción</label>
              <input type="date" value={form.fecha_infraccion} onChange={e => setForm(f => ({ ...f, fecha_infraccion: e.target.value }))}
                style={{ width: '100%', boxSizing: 'border-box', padding: '9px 12px', border: '1.5px solid var(--at-line)', borderRadius: '8px', fontSize: '14px', background: '#FAF7EF' }} />
            </div>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 600, color: '#3E5A4C', display: 'block', marginBottom: '4px' }}>Fecha límite descargo</label>
      ...
      {filtradas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', color: '#7E9389' }}>
          ...
          <p style={{ fontWeight: 600, color: '#7E9389' }}>No hay infracciones registradas</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {filtradas.map(i => {
            ...
            return (
              <div key={i.id} style={{ background: 'white', border: '1.5px solid var(--at-line)', borderRadius: '14px', padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '6px', alignItems: 'center' }}>
                      <span style={{ fontWeight: 700, fontSize: '13px', color: '#3E5A4C' }}>{tipo.label}</span>
                      {i.unidad_nombre && <span style={{ fontSize: '12.5px', color: '#1B3B36', fontWeight: 600 }}>📍 {i.unidad_nombre}</span>}
                    ...
                    <p style={{ margin: '0 0 6px', fontSize: '13.5px', color: '#3E5A4C' }}>{i.descripcion}</p>
                    <div style={{ fontSize: '12px', color: '#7E9389', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>

...
const CATEGORIA_CFG: Record<CategoriaSugerencia, { label: string; icon: string; color: string }> = {
  ...
  servicios:     { label: 'Servicios',     icon: '🧹', color: '#0369a1' },
...
const ESTADO_CFG: Record<EstadoSugerencia, { label: string; bg: string; color: string; next?: EstadoSugerencia }> = {
  ...
  en_revision: { label: 'En revisión', bg: '#dbeafe', color: '#2563eb', next: 'respondida' },
  respondida:  { label: 'Respondida',  bg: '#dcfce7', color: '#16a34a', next: 'archivada' },
  archivada:   { label: 'Archivada',   bg: '#f3f4f6', color: '#9ca3af' },
}
    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          { label: 'En revisión', val: enRevision, bg: '#dbeafe', color: '#2563eb' },
          { label: 'Respondidas', val: sugerencias.filter(s => s.estado === 'respondida').length, bg: '#dcfce7', color: '#16a34a' },
          { label: 'Total', val: sugerencias.length, bg: '#f9fafb', color: '#374151' },
        ].map(k => (
      ...
      {/* Formulario */}
      {mostrarForm && (
        <div style={{ background: '#f5f3ff', border: '1px solid #c4b5fd', borderRadius: 10, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <label htmlFor="anonima" style={{ fontSize: 13, color: '#374151', cursor: 'pointer' }}>Enviar de forma anónima</label>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
            ...
            {lista.map(s => {
              ...
              return (
                <div key={s.id} onClick={() => setSelected(s === selected ? null : s)}
                  style={{ background: selected?.id === s.id ? '#f5f3ff' : '#fff', border: `1.5px solid ${selected?.id === s.id ? '#a78bfa' : '#e5e7eb'}`, borderRadius: 10, padding: '12px 14px', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 3, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 15 }}>{cat.icon}</span>
                        <span style={{ fontWeight: 600, fontSize: 13, color: '#0f172a' }}>{s.titulo}</span>
                        <span style={{ padding: '1px 7px', borderRadius: 20, fontSize: 10, fontWeight: 700, background: est.bg, color: est.color }}>{est.label}</span>
                        {s.anonima && <span style={{ padding: '1px 7px', borderRadius: 20, fontSize: 10, background: '#f3f4f6', color: '#9ca3af' }}>Anónima</span>}
                      </div>
                      ...
                      <div style={{ fontSize: 12, color: '#374151', marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 500 }}>
                        {s.descripcion}
                      </div>
                    </div>
                  </div>
          ...
          {selected && (
            <div style={{ width: 280, flexShrink: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16, alignSelf: 'flex-start' }}>
              ...
              <div style={{ fontSize: 12, color: '#374151', background: '#f9fafb', borderRadius: 8, padding: '10px 12px', marginBottom: 10, lineHeight: 1.5 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12 }}>
                  {selected.estado === 'pendiente' && (
                    <button onClick={() => cambiarEstado(selected, 'en_revision')}
                      style={{ padding: '7px 0', background: '#dbeafe', color: '#2563eb', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                      🔍 Marcar en revisión
                    </button>
                  )}
                  ...
                  <button onClick={() => cambiarEstado(selected, 'archivada')}
                    style={{ padding: '7px 0', background: '#f3f4f6', color: '#9ca3af', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 12 }}>
                    📁 Archivar
                  </button>
                </div>
              )}

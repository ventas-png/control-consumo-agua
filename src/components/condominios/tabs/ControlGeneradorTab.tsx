...
const TIPOS: { value: TipoRegistroGenerador; label: string; icon: string; color: string }[] = [
  ...
  { value: 'mantenimiento',        label: 'Mantenimiento',       icon: '🔧', color: '#6366f1' },
  { value: 'prueba',               label: 'Prueba',              icon: '⚡', color: '#f59e0b' },
  { value: 'falla',                label: 'Falla',               icon: '⚠️', color: '#ef4444' },
  { value: 'arranque_emergencia',  label: 'Arranque emergencia', icon: '🚨', color: '#dc2626' },
]

const ESTADOS: { value: EstadoGenerador; label: string; color: string; bg: string; icon: string }[] = [
  { value: 'standby',      label: 'Standby',      color: '#10b981', bg: '#d1fae5', icon: '✅' },
  { value: 'operando',     label: 'Operando',      color: '#6366f1', bg: '#eef2ff', icon: '⚡' },
  { value: 'mantenimiento',label: 'Mantenimiento', color: '#f59e0b', bg: '#fef3c7', icon: '🔧' },
  { value: 'falla',        label: 'Falla',         color: '#ef4444', bg: '#fef2f2', icon: '⚠️' },
  { value: 'apagado',      label: 'Apagado',       color: '#6b7280', bg: '#f3f4f6', icon: '⭕' },
]
    ...
    <div style={{ padding: 16 }}>
      ...
      {generadores.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 12, marginBottom: 16 }}>
          {generadores.map(g => {
            ...
            return (
              <div key={g} style={{ background: est?.bg ?? '#f9fafb', border: `1px solid ${est?.color ?? '#e5e7eb'}`, borderRadius: 12, padding: '12px 16px' }}>
                ...
                {r ? (
                  <div>
                    {r.nivel_combustible_pct != null && (
                      <div style={{ marginBottom: 6 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                          <span style={{ fontSize: 11, color: '#6b7280' }}>Combustible</span>
                    ...
                    <div style={{ display: 'flex', gap: 10, fontSize: 11, flexWrap: 'wrap' }}>
                      {r.horas_acumuladas != null && <span style={{ color: '#6b7280' }}>⏱ {r.horas_acumuladas}h acum.</span>}
                      {r.voltaje != null && <span style={{ color: '#6366f1' }}>{r.voltaje}V</span>}
                      {r.frecuencia != null && <span style={{ color: '#6366f1' }}>{r.frecuencia}Hz</span>}
                    </div>
                    {r.proximo_mantenimiento && (
                      <div style={{ fontSize: 10, color: '#f59e0b', marginTop: 4 }}>🔧 Manto: {r.proximo_mantenimiento}</div>
                    )}
                  </div>
                ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 }}>
        <div style={{ background: '#eef2ff', borderRadius: 10, padding: '10px 14px', textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#6366f1' }}>{registros.length}</div>
          <div style={{ fontSize: 11, color: '#6366f1' }}>Registros totales</div>
        </div>
        <div style={{ background: '#fef3c7', borderRadius: 10, padding: '10px 14px', textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#f59e0b' }}>{totalHoras.toFixed(1)}h</div>
          <div style={{ fontSize: 11, color: '#f59e0b' }}>Horas operación totales</div>
        </div>
        <div style={{ background: fallas > 0 ? '#fef2f2' : '#f3f4f6', borderRadius: 10, padding: '10px 14px', textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: fallas > 0 ? '#ef4444' : '#6b7280' }}>{fallas}</div>
          <div style={{ fontSize: 11, color: fallas > 0 ? '#ef4444' : '#6b7280' }}>Fallas registradas</div>
        </div>
      </div>
        ...
        <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 10, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div>
              ...
              <input type="number" step="0.1" min="0" max="100" style={{ ...inp, borderColor: form.nivel_combustible_pct ? (parseFloat(form.nivel_combustible_pct) < 25 ? '#ef4444' : '#d1d5db') : '#d1d5db' }} value={form.nivel_combustible_pct} onChange={e => setForm(p => ({ ...p, nivel_combustible_pct: e.target.value }))} />
            </div>
            <div>
      ...
      {lista.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {lista.map(r => {
            ...
            const est = ESTADOS.find(e => e.value === r.estado)
            return (
              <div key={r.id} style={{ background: '#fff', border: `1px solid ${r.tipo === 'falla' ? '#fecaca' : '#e5e7eb'}`, borderRadius: 8, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                ...
                <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                  {r.nivel_combustible_pct != null && (
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 10, color: '#9ca3af' }}>Combustible</div>
                  ...
                  {r.horas_operacion != null && (
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 10, color: '#9ca3af' }}>Horas</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#374151' }}>{r.horas_operacion}h</div>
                    </div>
                  )}
                  {r.voltaje != null && (
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 10, color: '#9ca3af' }}>Voltaje</div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#6366f1' }}>{r.voltaje}V</div>
                    </div>
                  )}
                </div>
              </div>

...
const ESTADO_CFG: Record<EstadoUnidad, { label: string; color: string; bg: string; border: string; icon: string }> = {
  ...
  arrendada: { label: 'Arrendada',  color: '#2563eb', bg: '#eff6ff', border: '#93c5fd', icon: '🔵' },
  vacia:     { label: 'Sin cuotas', color: '#9ca3af', bg: '#f9fafb', border: '#e5e7eb', icon: '⬜' },
}
    ...
    <div style={{ padding: 16 }} onClick={() => setPopover(null)}>
        ...
        {(Object.keys(ESTADO_CFG) as EstadoUnidad[]).map(e => {
          ...
          return (
            <div key={e} onClick={ev => { ev.stopPropagation(); setFiltro(filtro === e ? '' : e) }}
              style={{ background: filtro === e ? cfg.bg : '#fff', border: `1.5px solid ${filtro === e ? cfg.border : '#e5e7eb'}`, borderRadius: 10, padding: '10px 14px', cursor: 'pointer', transition: 'all 0.15s' }}>
      ...
      {filtro && (
        <div style={{ marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 12, color: '#6b7280' }}>Filtrando: <strong>{ESTADO_CFG[filtro].label}</strong> ({filtradas.length} unidades)</span>
          <button onClick={() => setFiltro('')} style={{ fontSize: 11, padding: '2px 8px', border: '1px solid #e5e7eb', borderRadius: 5, cursor: 'pointer', background: '#f9fafb' }}>✕ Limpiar</button>
        </div>
      )}
              ...
              <div key={info.unidad.id} style={{ position: 'relative' }}>
                ...
                {isOpen && popInfo && (
                  <div
                    ...
                    <div style={{ fontWeight: 700, fontSize: 14, color: '#0f172a', marginBottom: 6, borderBottom: '1px solid #f1f5f9', paddingBottom: 4 }}>
                      {info.unidad.nombre}
                    </div>
                    <div style={{ fontSize: 11, display: 'flex', flexDirection: 'column', gap: 4 }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ color: '#6b7280' }}>Cuotas totales</span>
                        <span style={{ fontWeight: 600 }}>{info.cuotasTotales}</span>
                      </div>
                      {info.cuotasVencidas > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6b7280' }}>Cuotas vencidas</span>
                          <span style={{ fontWeight: 700, color: '#ef4444' }}>{info.cuotasVencidas}</span>
                        </div>
                      )}
                      <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #f1f5f9', paddingTop: 4, marginTop: 2 }}>
                      ...
                      {info.arrendatario && (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: '#6b7280' }}>Arrendatario</span>
                          <span style={{ fontWeight: 600, color: '#2563eb', maxWidth: 120, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{info.arrendatario}</span>
                        </div>
                      )}
                      {info.unidad.tipo && (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>

    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 8, marginBottom: 16 }}>
        {SECCIONES.map(s => {
          ...
          return (
            <div key={s.id} style={{ background: '#f9fafb', borderRadius: 8, padding: '8px 12px', textAlign: 'center', cursor: 'pointer', border: `1.5px solid ${filtroSeccion === s.id ? '#2563eb' : '#e5e7eb'}` }}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: '#6b7280' }}>{lista.length} artículo{lista.length !== 1 ? 's' : ''} · {totalActivos} activos</span>
          {canEdit && (
            ...
            return (
              <div key={sec.id} style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {items.map(a => (
                    <div key={a.id} style={{ background: '#fff', border: `1px solid ${a.activo ? '#e5e7eb' : '#fde68a'}`, borderRadius: 8, overflow: 'hidden', opacity: a.activo ? 1 : 0.7 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', cursor: 'pointer' }}
                        ...
                        <span style={{ fontSize: 10, color: '#9ca3af' }}>#{a.orden}</span>
                        {canEdit && (
                          <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                            <button onClick={() => toggleActivo(a.id, a.activo)}
                              style={{ padding: '3px 8px', background: '#f3f4f6', border: 'none', borderRadius: 5, cursor: 'pointer', fontSize: 10, color: '#6b7280' }}>
                        ...
                        <span style={{ fontSize: 11, color: '#9ca3af' }}>{expandido === a.id ? '▲' : '▼'}</span>
                      </div>
                      {expandido === a.id && (
                        <div style={{ padding: '0 14px 14px', fontSize: 13, color: '#374151', whiteSpace: 'pre-wrap', lineHeight: 1.6, borderTop: '1px solid #f3f4f6', paddingTop: 10 }}>
                          {a.contenido}
                        </div>
                      )}
                    </div>

...
export function SostenibilidadTab({ residuos, proyectoId, companyId }: Props) {
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      <h2 style={{ margin: '0 0 20px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Dashboard de Sostenibilidad</h2>

      {/* KPIs globales */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        {[
          { label: 'Agua consumida (6m)', value: `${totalM3Agua.toFixed(0)} m³`, icon: '💧', color: '#0ea5e9' },
          { label: 'CO₂ equiv. agua',     value: `${co2Agua.toFixed(0)} kg`,     icon: '🌊', color: '#7c3aed' },
          { label: 'Residuos totales',    value: `${totalKg.toFixed(0)} kg`,     icon: '🗑️', color: '#64748b' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
        ...
        <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '20px' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>💧 Consumo de Agua (últimos 6 meses)</h3>
          {loadingAgua ? (
            <div style={{ textAlign: 'center', color: '#94a3b8', padding: '20px' }}>Cargando...</div>
          ) : aguaMeses.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#94a3b8', padding: '20px', fontSize: '13px' }}>Sin datos de consumo de agua</div>
          ) : (
              ...
              {aguaMeses.map(a => (
                <div key={a.mes}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#64748b', marginBottom: '3px' }}>
                    ...
                    <span style={{ fontWeight: 700, color: '#0ea5e9' }}>{a.m3.toFixed(1)} m³</span>
                  </div>
                  <div style={{ background: '#f1f5f9', borderRadius: '4px', height: '8px' }}>
                    <div style={{ background: '#0ea5e9', height: '8px', borderRadius: '4px', width: barWidth(a.m3, maxAgua), transition: 'width 0.3s' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Residuos por mes */}
        <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '20px' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>♻️ Residuos por Mes (últimos 6 meses)</h3>
          {residuosMeses.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#94a3b8', padding: '20px', fontSize: '13px' }}>Sin datos de residuos</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {residuosMeses.map(r => (
                <div key={r.mes}>
        ...
        {/* Residuos por tipo */}
        <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '20px' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>🗑️ Residuos por Tipo</h3>
          {tipoEntries.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#94a3b8', padding: '20px', fontSize: '13px' }}>Sin datos</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {tipoEntries.map(([tipo, kg]) => (
                <div key={tipo}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#64748b', marginBottom: '3px' }}>
                    ...
                    <span style={{ fontWeight: 700, color: TIPO_RECICLABLE.has(tipo) ? '#10b981' : '#94a3b8' }}>{kg.toFixed(1)} kg</span>
                  </div>
                  <div style={{ background: '#f1f5f9', borderRadius: '4px', height: '8px' }}>
                    <div style={{ background: TIPO_RECICLABLE.has(tipo) ? '#10b981' : '#94a3b8', height: '8px', borderRadius: '4px', width: barWidth(kg, maxTipo), transition: 'width 0.3s' }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            {[
              ...
              <div key={item.label} style={{ background: 'white', borderRadius: '8px', padding: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#0f172a' }}>{item.icon} {item.label}</div>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>{item.detail}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '16px', fontWeight: 800, color: item.saved ? '#16a34a' : '#64748b' }}>

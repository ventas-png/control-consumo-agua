...
const COLOR = {
  residente: { bg: '#eff6ff', color: '#2563eb', border: '#bfdbfe', dot: '#2563eb' },
  personal:  { bg: '#f5f3ff', color: '#7c3aed', border: '#ddd6fe', dot: '#7c3aed' },
}
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: '1100px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>🎂 Cumpleaños</h2>
          ...
          {(['todos', 'residente', 'personal'] as const).map(t => (
            <button key={t} onClick={() => setFiltroTipo(t)}
              style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', border: '1.5px solid',
                ...
                background: filtroTipo === t ? '#eff6ff' : 'white',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'Próximos 30d', val: proximos30.length, icon: '🗓️', color: '#0ea5e9', bg: '#e0f2fe' },
          { label: 'Registrados', val: todos.length, icon: '👤', color: '#7c3aed', bg: '#f5f3ff' },
        ].map(k => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 290px', gap: '20px', alignItems: 'start' }}>
        ...
        <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '14px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
            <button onClick={prevMes} style={{ padding: '6px 14px', background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '16px', color: '#374151' }}>‹</button>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontWeight: 700, fontSize: '15px', color: '#0f172a' }}>{MESES[mes]} {anio}</span>
              {(mes !== hoy.getMonth() || anio !== hoy.getFullYear()) && (
                <button onClick={irHoy} style={{ padding: '3px 10px', background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: '6px', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>Hoy</button>
              )}
            </div>
            <button onClick={nextMes} style={{ padding: '6px 14px', background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '16px', color: '#374151' }}>›</button>
          </div>
          {/* Encabezados días */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid #e2e8f0' }}>
            {DIAS.map(d => (
              <div key={d} style={{ textAlign: 'center', padding: '8px 2px', fontSize: '11px', fontWeight: 700, color: '#64748b' }}>{d}</div>
            ))}
          </div>
          {/* Celdas */}
            ...
            {cells.map((d, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  background: hoyFlag ? '#eff6ff' : lista.length > 0 ? '#fafafa' : 'white',
                }}>
                  {d && (
                    <>
                      <div style={{
                        ...
                        color: hoyFlag ? 'white' : lista.length > 0 ? '#0f172a' : '#94a3b8',
                        background: hoyFlag ? '#2563eb' : 'transparent',
                      }}>{d}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        {lista.length > 3 && (
                          <div style={{ fontSize: '9px', color: '#94a3b8', fontWeight: 600 }}>+{lista.length - 3} más</div>
                        )}
                      </div>
                    </>
                  )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontWeight: 700, fontSize: '12px', color: '#0f172a', marginBottom: '8px' }}>Leyenda</div>
            {[
          ...
          {/* Próximos 30 días */}
          <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontWeight: 700, fontSize: '12px', color: '#0f172a', marginBottom: '10px' }}>
              📅 Próximos 30 días
              {proximos30.length > 0 && <span style={{ marginLeft: 6, background: '#e0f2fe', color: '#0ea5e9', borderRadius: '12px', padding: '1px 8px', fontSize: '11px' }}>{proximos30.length}</span>}
            </div>
            {proximos30.length === 0 ? (
              <p style={{ fontSize: '12px', color: '#94a3b8', textAlign: 'center', margin: '16px 0' }}>Sin cumpleaños en los próximos 30 días</p>
            ) : (
                ...
                {proximos30.map(c => {
                  ...
                  return (
                    <div key={c.id} style={{
                      ...
                      background: c.dias === 0 ? '#fef2f2' : c.dias <= 7 ? '#fff7ed' : '#f8fafc',
                      borderRadius: '8px', border: `1px solid ${c.dias === 0 ? '#fca5a5' : c.dias <= 7 ? '#fed7aa' : '#e2e8f0'}`,
                    }}>
                      ...
                      <div style={{ flexShrink: 0, textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: '10px', fontWeight: 700, color: c.dias === 0 ? '#dc2626' : c.dias <= 7 ? '#ea580c' : '#64748b' }}>

        ...
        {[
          ...
          <div key={s.n} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: paso >= s.n ? '#2563eb' : '#e5e7eb', color: paso >= s.n ? '#fff' : '#9ca3af', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700 }}>
                {paso > s.n ? '✓' : s.n}
              </div>
              <div style={{ fontSize: 10, color: paso >= s.n ? '#2563eb' : '#9ca3af', fontWeight: 600, marginTop: 3 }}>{s.label}</div>
            </div>
            {i < 2 && <div style={{ height: 2, flex: 1, background: paso > s.n ? '#2563eb' : '#e5e7eb', margin: '0 4px', marginBottom: 16 }} />}
          </div>
        ))}
      </div>
                ...
                {plantillasActivas.map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => setPlantillaId(p.id)}
                      style={{ padding: '10px 12px', border: `1.5px solid ${sel ? c.color : '#e5e7eb'}`, borderRadius: 8, cursor: 'pointer', background: sel ? c.color + '11' : '#f9fafb' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: c.color }}>{c.icon} {c.label}</span>
                      </div>
                      <div style={{ fontWeight: 600, fontSize: 12, color: '#0f172a' }}>{p.nombre}</div>
                      <div style={{ fontSize: 10, color: '#9ca3af', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.cuerpo.slice(0, 60)}…</div>
                    </div>
                  )
                })}
              </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16 }}>
              ...
              {(Object.keys(SEGMENTO_CFG) as SegmentoEnvio[]).map(s => {
                ...
                return (
                  <div key={s} onClick={() => setSegmento(s)}
                    style={{ padding: '10px 12px', border: `1.5px solid ${sel ? '#2563eb' : '#e5e7eb'}`, borderRadius: 8, cursor: 'pointer', background: sel ? '#eff6ff' : '#f9fafb', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 18 }}>{cfg.icon}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 12, color: sel ? '#1d4ed8' : '#0f172a' }}>{cfg.label}</div>
                      <div style={{ fontSize: 10, color: '#9ca3af' }}>{cfg.desc}</div>
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: sel ? '#2563eb' : '#6b7280' }}>{count}</div>
                  </div>
                )
              })}
            </div>
            ...
            <div style={{ marginTop: 14 }}>
              <button onClick={() => setPaso(2)} disabled={!plantillaId || destinatarios.length === 0}
                style={{ width: '100%', padding: '10px 0', background: !plantillaId || destinatarios.length === 0 ? '#94a3b8' : '#2563eb', color: '#fff', border: 'none', borderRadius: 8, cursor: !plantillaId || destinatarios.length === 0 ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 700 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16, marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setPaso(1)} style={{ padding: '6px 14px', border: '1px solid #e5e7eb', borderRadius: 7, cursor: 'pointer', fontSize: 12, background: '#f9fafb' }}>← Volver</button>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
              {previews.map(p => (
                <button key={p.unidad.id} onClick={() => setPreviewUnidadId(p.unidad.id)}
                  style={{ padding: '4px 10px', border: `1px solid ${previewUnidad?.id === p.unidad.id ? canalCfg?.color ?? '#2563eb' : '#e5e7eb'}`, borderRadius: 6, cursor: 'pointer', fontSize: 11, background: previewUnidad?.id === p.unidad.id ? (canalCfg?.color ?? '#2563eb') + '11' : '#f9fafb' }}>
                  {p.unidad.nombre}
                </button>
              ))}
              {destinatarios.length > 5 && <span style={{ fontSize: 11, color: '#9ca3af', alignSelf: 'center' }}>+{destinatarios.length - 5} más</span>}
            </div>

...
const CATEGORIAS: { value: CategoriaNota; label: string; icon: string; color: string }[] = [
  ...
  { value: 'seguimiento',   label: 'Seguimiento',   icon: '🔄', color: '#6366f1' },
  { value: 'reunion',       label: 'Reunión',       icon: '👥', color: '#10b981' },
  { value: 'otro',          label: 'Otro',          icon: '📌', color: '#8b5cf6' },
]
  ...
  function NotaCard({ n, highlight = false }: { n: NotaAdmin; highlight?: boolean }) {
    ...
    return (
      <div onClick={() => { setSelected(n); setMostrarForm(false) }}
        style={{ background: highlight ? '#fffbeb' : n.resuelta ? '#f9fafb' : '#fff', border: `1px solid ${selected?.id === n.id ? '#6366f1' : highlight ? '#fde68a' : '#e5e7eb'}`, borderRadius: 10, padding: '12px 14px', cursor: 'pointer', opacity: n.resuelta ? 0.65 : 1, marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6, alignItems: 'center' }}>
          ...
          {dias !== null && (
            <span style={{ fontSize: 10, color: dias < 0 ? '#ef4444' : dias === 0 ? '#f59e0b' : '#6b7280' }}>
      ...
      <div style={{ width: 320, borderRight: '1px solid #e5e7eb', overflowY: 'auto', flexShrink: 0 }}>
        ...
        <div style={{ padding: '10px 12px' }}>
          ...
          {lista.length === 0 && !fijadas.length && (
            <div style={{ textAlign: 'center', color: '#9ca3af', padding: '32px 0', fontSize: 13 }}>Sin notas</div>
          )}
        ...
        {!mostrarForm && selected && (() => {
          ...
          return (
            <div>
                ...
                {canEdit && !selected.resuelta && (
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={() => toggleFijada(selected)}
                      style={{ padding: '6px 12px', background: selected.fijada ? '#fef3c7' : '#f3f4f6', color: selected.fijada ? '#f59e0b' : '#374151', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 12 }}>
              ...
              {dias !== null && (
                <div style={{ background: dias < 0 ? '#fef2f2' : dias === 0 ? '#fef3c7' : '#eef2ff', border: `1px solid ${dias < 0 ? '#fecaca' : dias === 0 ? '#fde68a' : '#c7d2fe'}`, borderRadius: 8, padding: '10px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 16 }}>⏰</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: dias < 0 ? '#ef4444' : dias === 0 ? '#f59e0b' : '#6366f1' }}>
                      {dias < 0 ? `Recordatorio vencido hace ${Math.abs(dias)} días` : dias === 0 ? 'Recordatorio: hoy' : `Recordatorio en ${dias} días`}
                    </div>
                    <div style={{ fontSize: 11, color: '#6b7280' }}>{selected.fecha_recordatorio}</div>
                  </div>
                </div>
              )}

              <div style={{ background: '#f9fafb', borderRadius: 10, padding: '16px 20px', fontSize: 14, lineHeight: 1.7, color: '#374151', whiteSpace: 'pre-wrap', marginBottom: 16 }}>
                {selected.contenido}
              </div>

...
const ESTADO_CONFIG: Record<EstadoHousekeeping, { label: string; color: string; bg: string }> = {
  ...
  en_proceso: { label: 'En proceso', color: '#0ea5e9', bg: '#e0f2fe' },
    ...
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'En proceso',       value: String(enProceso),                                                      icon: '🔄', color: '#0ea5e9' },
          { label: 'Completados (mes)',value: String(completados),                                                    icon: '✅', color: '#10b981' },
          { label: 'Costo del mes',    value: costoMes > 0 ? `${moneda} ${costoMes.toFixed(0)}` : '—',               icon: '💰', color: '#8b5cf6' },
        ].map(k => (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Housekeeping</h2>
        {canCreate && !showForm && (
          <button onClick={() => setShowForm(true)} style={{ padding: '8px 16px', background: '#0ea5e9', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>+ Nuevo Servicio</button>
        )}
      </div>
            ...
            <div key={fecha}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 700, whiteSpace: 'nowrap',
                  ...
                  border: `1.5px solid ${fecha === today ? '#fcd34d' : '#e2e8f0'}`,
                  color: fecha === today ? '#92400e' : '#64748b' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: '10px' }}>
                {byFecha[fecha].map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: '10px', padding: '14px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                        <div style={{ fontWeight: 700, fontSize: '14px', color: '#0f172a' }}>{tipo.icon} {tipo.label}</div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', fontSize: '12px', color: '#64748b', marginBottom: '10px' }}>
                        ...
                        {s.costo && <div style={{ fontWeight: 600, color: '#0f172a' }}>💰 {moneda} {s.costo.toFixed(2)}</div>}
                      </div>
                      {canEdit && (
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                          {s.estado === 'pendiente' && (
                            <button onClick={() => handleEstado(s.id, 'en_proceso')} style={{ flex: 1, padding: '4px 8px', background: '#e0f2fe', color: '#0369a1', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>Iniciar</button>
                          )}
                          {s.estado === 'en_proceso' && (
                          ...
                          <button onClick={() => startEdit(s)} style={{ padding: '4px 8px', background: '#f1f5f9', border: 'none', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>✏️</button>

...
export function InspeccionesTab({ inspecciones, proyectoId, companyId, canCreate, canEdit, onRefresh }: Props) {
  ...
  async function handleSave() {
    if (!form.fecha) return Swal.fire('Campo requerido', 'Ingresa la fecha de inspección.', 'warning')
    setSaving(true)
    const payload = {
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Inspecciones Normativas</h2>
        {canCreate && !showForm && (
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {filtered.map(ins => {
            ...
            return (
              <div key={ins.id} style={{ background: 'white', border: `1.5px solid ${ins.resultado === 'reprobado' ? '#fca5a5' : '#e2e8f0'}`, borderRadius: '10px', padding: '14px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontSize: '24px' }}>{ti.icon}</span>
                    <div>
                      <div style={{ fontWeight: 700, color: '#0f172a' }}>{ti.label}</div>
                      {ins.entidad_inspectora && <div style={{ fontSize: '12px', color: '#64748b' }}>{ins.entidad_inspectora}</div>}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    {ins.certificado_url && (
                      <a href={ins.certificado_url} target="_blank" rel="noreferrer" style={{ padding: '4px 8px', background: '#f1f5f9', borderRadius: '6px', fontSize: '12px', textDecoration: 'none', color: '#374151' }}>📄</a>
                    )}
                    {canEdit && <button onClick={() => startEdit(ins)} style={{ padding: '4px 8px', background: '#f1f5f9', border: 'none', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>✏️</button>}
                ...
                {ins.hallazgos && (
                  <div style={{ marginTop: '10px', padding: '8px 12px', background: '#f8fafc', borderRadius: '8px', fontSize: '12px', color: '#64748b' }}>
                ...
                {ins.fecha_proxima && (
                  <div style={{ marginTop: '6px', fontSize: '12px', color: proximaAlerta ? '#92400e' : '#64748b', fontWeight: proximaAlerta ? 600 : 400 }}>

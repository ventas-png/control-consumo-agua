  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--at-ink-2)' }}>
          Rubros{' '}
          <span style={{ fontSize: 11, fontWeight: 400, color: 'var(--at-ink-3)' }}>
      ...
      {rubros.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '20px 0', fontSize: 12, color: 'var(--at-ink-3)', border: '1.5px dashed var(--at-line)', borderRadius: 8 }}>
          Sin rubros. Agrega al menos uno para continuar.
        </div>
      ) : (
        <div style={{ border: '1px solid var(--at-line)', borderRadius: 8, overflow: 'hidden' }}>
          {/* Header de columnas */}
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 130px 150px 32px', gap: 0, background: 'var(--at-surface-2)', borderBottom: '1px solid var(--at-line)', padding: '6px 10px' }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--at-ink-3)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Nombre del rubro</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--at-ink-3)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Método</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--at-ink-3)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Valor ({monedaLabel.trim() || '—'})</span>
            <span />
          </div>
            ...
            return (
              ...
              >
                ...
                <select
                  ...
                  disabled={disabled}
                  style={{ width: '100%', padding: '6px 6px', fontSize: 12, fontWeight: 600, border: '1px solid var(--at-line)', borderRadius: 6, background: disabled ? 'var(--at-surface-2)' : badge.bg, color: badge.color, cursor: disabled ? 'default' : 'pointer', marginRight: 8 }}
                >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginRight: 4 }}>
                  {moneda && <span style={{ fontSize: 12, color: 'var(--at-ink-3)', flexShrink: 0 }}>{moneda}</span>}
                  <input
                    type="number"
                    min={0}
                    step={0.01}
      ...
      {rubros.some(r => r.metodo !== 'fijo') && (
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          {rubros.some(r => r.metodo === 'alicuota') && (
            <p style={{ margin: 0, fontSize: 11, color: 'var(--at-accent-hover)' }}>

    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '16px', marginBottom: '28px' }}>
        {[
          ...
          { label: 'Convenios Activos', value: convenios.filter(c => c.estado === 'activo').length.toString(), icon: '🤝', bg: 'linear-gradient(135deg,#8b5cf6,#7c3aed)', },
        ].map((s, i) => (
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '20px', alignItems: 'center' }}>
            <input
              ...
              style={{ flex: 1, minWidth: '200px', padding: '10px 14px', borderRadius: '8px', border: '1.5px solid #e2e8f0', fontSize: '14px', fontFamily: 'inherit' }}
            />
            <select
              value={filtroEstado}
            ...
            {selectedRows.size > 0 && canEdit && (
              <>
                ...
                <button onClick={abrirConvenioGrupal} style={{
                  padding: '10px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer',
                  background: '#ede9fe', color: '#7c3aed', fontWeight: 600, fontSize: '13px',
                }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                ...
                <tbody>
                  {registrosFiltrados.length === 0 ? (
                    <tr><td colSpan={canEdit ? 8 : 7} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                    ...
                    return (
                      <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9', background: selectedRows.has(r.id) ? '#f0f9ff' : undefined }}
                        onMouseEnter={e => { if (!selectedRows.has(r.id)) (e.currentTarget as HTMLTableRowElement).style.background = '#f8fafc' }}
...
function ConveniosLista({ convenios, clientes, moneda, canEdit, onRefresh }: ConveniosListaProps) {
  const ESTADO_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
    ...
    completado: { label: 'Completado',  bg: '#dbeafe', color: '#1d4ed8' },
  ...
  return (
    <div style={{ display: 'grid', gap: '12px' }}>
      {convenios.map(conv => {
        ...
        return (
          <div key={conv.id} style={{ background: 'white', borderRadius: '12px', padding: '20px', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
              <div>
                ...
                {conv.descripcion && <div style={{ fontSize: '13px', color: '#64748b', marginBottom: '8px' }}>{conv.descripcion}</div>}
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                {canEdit && conv.estado === 'activo' && (
                  <div style={{ display: 'flex', gap: '6px', marginTop: '8px', justifyContent: 'flex-end' }}>
                    <button onClick={() => cambiarEstado(conv.id, 'completado')} style={{ padding: '5px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer', background: '#dbeafe', color: '#1d4ed8', fontSize: '12px', fontWeight: 600 }}>
                      ✓ Completar
                    </button>
            ...
            <div style={{ marginTop: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '6px' }}>
                <span style={{ color: '#64748b' }}>Pagado: <strong style={{ color: '#10b981' }}>{moneda} {conv.monto_pagado.toFixed(2)}</strong></span>
                <span style={{ color: '#64748b' }}>Total: <strong style={{ color: '#0f172a' }}>{moneda} {conv.monto_total.toFixed(2)}</strong></span>
              </div>
              <div style={{ height: '8px', background: '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.min(pct, 100)}%`, background: pct >= 100 ? '#10b981' : '#0ea5e9', borderRadius: '4px', transition: 'width 0.3s' }} />
              </div>
              <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '4px', textAlign: 'right' }}>{pct.toFixed(1)}% completado</div>
            </div>
          </div>
        )
      })}

...
// ============================================================================
// Muestra rows con deleted_at != NULL en las 5 tablas con soft delete (F2.7).
// Cada row tiene boton "Restaurar" que ejecuta UPDATE deleted_at = NULL.
//
// El restore queda automaticamente registrado en audit_log via el trigger
// audit_trigger_func (F2.7) — no hace falta lógica adicional para trazabilidad.
//
// Permisos: la policy SELECT de audit_log + RLS de cada tabla restringen ya
// la visibilidad por tenant. UPDATE en estas tablas tambien tiene RLS — solo
// admin/owner del tenant pueden ejecutarlo.
//
// Scope MVP:
//   - Filtros: tabla (5 valores)
//   - Paginacion 25 rows/pagina (rows menos frecuentes que audit_log)
//   - Restore individual (no bulk en este PR)
//   - Resolucion deleted_by → full_name
//   - JSON viewer del row entero (para que el admin valide antes de restore)

interface Props { onClose: () => void; defaultTable?: SoftTable }

type SoftTable = 'pagos' | 'cuotas_condominio' | 'fondo_reserva_condominio' | 'gastos_condominio' | 'tickets_mantenimiento'
...
const TABLES: SoftTable[] = ['pagos', 'cuotas_condominio', 'fondo_reserva_condominio', 'gastos_condominio', 'tickets_mantenimiento']

// Columna timestamp para ordenamiento secundario por tabla (cuando deleted_at
// empata). Default created_at, pero algunas tablas tienen otro nombre comun.
const ORDER_COLUMNS: Record<SoftTable, string> = {
  pagos:                    'created_at',
  cuotas_condominio:        'created_at',
  fondo_reserva_condominio: 'fecha',
  gastos_condominio:        'fecha',

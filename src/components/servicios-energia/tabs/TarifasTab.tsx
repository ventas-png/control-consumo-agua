    ...
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        <div style="text-align:left;max-height:480px;overflow-y:auto;padding-right:4px;">
          ...
          <input id="t_nombre" placeholder="Tarifa BT1" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />

          ${proyectos.length > 1 ? `
          <label style="display:block;margin:0.75rem 0 0.2rem;font-weight:bold;">Proyecto *</label>
          <select id="t_project" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;">
          ...
          <textarea id="t_desc" rows="2" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;"></textarea>

          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;margin-top:0.75rem;">
            <div>
              <label style="display:block;margin-bottom:0.2rem;font-size:0.9rem;">Precio kWh energía</label>
              <input id="t_kwh_e" type="number" step="0.000001" value="0" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />
            </div>
            <div>
              <label style="display:block;margin-bottom:0.2rem;font-size:0.9rem;">Precio kW potencia</label>
              <input id="t_kw_p" type="number" step="0.0001" value="0" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />
            </div>
            <div>
              <label style="display:block;margin-bottom:0.2rem;font-size:0.9rem;">Cargo fijo (${moneda})</label>
              <input id="t_fijo" type="number" step="0.01" value="0" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />
            </div>
            <div>
              <label style="display:block;margin-bottom:0.2rem;font-size:0.9rem;">Alumbrado público</label>
              <input id="t_alumbrado" type="number" step="0.0001" value="0" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />
            </div>
            <div>
              <label style="display:block;margin-bottom:0.2rem;font-size:0.9rem;">Tipo alumbrado</label>
              <select id="t_alumbrado_tipo" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;">
            ...
            <div>
              ...
              <input id="t_iva" type="number" step="0.01" value="12" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />
            </div>
            <div>
              <label style="display:block;margin-bottom:0.2rem;font-size:0.9rem;">Precio kWh exportado</label>
              <input id="t_kwh_exp" type="number" step="0.000001" value="0" style="width:100%;padding:0.45rem;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;" />
            </div>
          </div>
        </div>
      `,
  ...
  return (
    <div>
      ...
      {tarifasEnergia.length === 0 ? (
        <p style={{ color: '#888', fontStyle: 'italic', marginTop: '1.5rem' }}>No hay tarifas registradas</p>
      ) : (
              ...
              return (
                <tr key={t.id} style={{ borderBottom: '1px solid #eee' }}>
                  ...
                  <td style={{ padding: '0.5rem' }}>
                    <span style={{ color: t.activa ? '#28a745' : '#dc3545' }}>{t.activa ? '● Activa' : '● Inactiva'}</span>
                  </td>
                  {canEdit && (
                    <td style={{ padding: '0.5rem', textAlign: 'center' }}>
                      <button onClick={() => handleStartEdit(t)} style={{ marginRight: '0.5rem', padding: '0.25rem 0.75rem', backgroundColor: '#0066cc', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer' }}>Editar</button>
                      <button onClick={() => handleDelete(t.id, t.nombre)} style={{ padding: '0.25rem 0.75rem', backgroundColor: '#dc3545', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer' }}>Eliminar</button>
                    </td>
                  )}
                </tr>
              )

...
function AdminDashboardStatsImpl({ registros, moneda, clientes, fechaDesde, fechaHasta, isLoading = false }: Props) {
  ...
  const stats = [
    ...
    },
    {
      ...
      unit: moneda,
      icon: '💰',
      bg: 'linear-gradient(135deg, var(--at-success) 0%, var(--at-success-strong) 100%)',
    },
    {
      label: 'Pendientes',
      value: `${pendientes}`,
      unit: 'lecturas',
      icon: '⏳',
      bg: 'linear-gradient(135deg, var(--at-warning) 0%, var(--at-warning) 100%)',
    },
    {
      label: 'En Mora',
      value: `${enMora}`,
      unit: 'pagos',
      icon: '⚠️',
      bg: 'linear-gradient(135deg, var(--at-danger) 0%, var(--at-danger) 100%)',
    },
    {
      label: 'Pagados',
      value: `${pagados}`,

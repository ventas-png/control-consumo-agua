...
| 5 | **`useAuth.ts` (524 L)** mezcla sesión, permisos y empresa — *RESUELTO 2026-06-11*: 524 → 82 L como orquestador; responsabilidades en `hooks/auth/{useOAuthSession,useSessionMaintenance,useCredentialsLogin}.ts` (bootstrap/OAuth, refresh+realtime RBAC+expiración, login+MFA) con la API pública intacta | Pendiente P13 del tracker |
...
| 8 | E2E limitado: 6 happy paths en `e2e/` — *ATENDIDO 2026-06-11*: +2 specs con el mismo patrón credencial-gated: `role-restricted-access.e2e.ts` (rol viewer/operator ve "Acceso Denegado" en las 5 rutas admin del registro declarativo; requiere secretos `E2E_RESTRICTED_EMAIL/PASSWORD` nuevos) y `agua-lectura-validaciones.e2e.ts` (edge cases de dinero: guardar sin unidad y consumo negativo rechazados). Las violaciones RLS a nivel de datos ya las cubre el harness server-side (83 verificaciones) | Falta cobertura de violaciones RLS, edge cases de dinero |

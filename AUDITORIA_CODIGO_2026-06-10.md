...
| # | Hallazgo | Evidencia |
|---|----------|-----------|
| 3 | **Componentes monolíticos** (top por LOC) — *AmenidadesTab, VisitantesTab, CustomerPortal, ClientesSection, UnidadesSection, RutasSection y SeguridadTab RESUELTOS 2026-06-11*: 1,932 → ~710 L, 1,530 → 525 L, 1,511 → 485 L, 1,302 → 625 L, 1,226 → 542 L, 1,169 → 416 L y 1,128 → 386 L respectivamente (reglas puras en lib con tests + vistas/modales extraídos con ctx tipado, JSX intacto); resta: `ContadoresSection.tsx` (1069) | Mantenimiento costoso, alto riesgo en cada cambio. El refactor de `EmpresaSection` (1259 → 112 LOC + domain) es el patrón a replicar |
